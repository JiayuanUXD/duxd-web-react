...
但在实际业务场景中可能出现需要部分页面或者页面的部分内容呈现暗黑样式（尽管少，但确实存在），此时可以通过修改 `less` 变量 `ddmc-theme-tag` 的值来改变暗黑样式的应用位置，实现以下效果
...
### 在 `less-loader` 中配置覆盖变量 `ddmc-theme-tag` 的值。

例如配置 ` ddmc-theme-tag  `为 `.ddmc-theme`。
...
在body标签上添加类名 `ddmc-theme`，并在想要应用暗黑样式的标签上设置类名 `ddmc-theme` 以及添加属性 `ddmc-theme=dark`，即可实现如下图所示的页面部分呈现暗黑样式的效果。

p.s: 在body上添加类名是因为所有的css变量都定义在`.ddmc-theme`上，如果不设置将会出现样式展示问题。
