...
* Single package component: Take `@ddmc-materials/select-with-check-all` as an example, its package contains only one component `SelectWithCheckAll`, and the material ID corresponding to this component is `@ddmc-materials/select-with-check-all/SelectWithCheckAll`;

* Component library: Take `@ddmc-design/web-react` as an example, its package contains multiple components, and the material ID corresponding to its components is `@ddmc-design/web-react/ComponentName`.
...
```JavaScript
import SelectWithCheckAll from '@ddmc-materials/select-with-check-all';
```

## Initialize the workspace
Initialize a React component workspace with the following command:
```Bash
npx @ddmc-cli/generator new workspace-name
```

![](https://p9-ddmcsite.byteimg.com/tos-cn-i-goo7wpa0wc/0740ed5120314888b09f8db9e0cb0ac6~tplv-goo7wpa0wc-image.image)

## Add new materials
If you have previously installed the ddmc-cli 1.x NPM package globally, when running the `ddmc xxx` command, Node may hit the 1.x CLI command. In this case, you can use `npm uninstall ddmc-cli -g` to uninstall the 1.x CLI, or use `npx ddmc xxx` to specify Node to look for executable commands in the current project's node_modules first.

Enter the workspace created in the previous step and create a blank component with the following command:
```Bash
## Create a blank component
npx ddmc create ComponentName
```
...
```Bash
$ cd ddmc-cli-next-demo
$ npm start
```

Execute the command in the console and you will get the following output:
![](https://p9-ddmcsite.byteimg.com/tos-cn-i-goo7wpa0wc/7d050ccc02984412aec3cb1de82cbf65~tplv-goo7wpa0wc-image.image)

And automatically open the workspace page in the browser:
![](https://p9-ddmcsite.byteimg.com/tos-cn-i-goo7wpa0wc/4cf1384a2d0b4990821953d8b811ec59~tplv-goo7wpa0wc-image.image)
...
<div data-ddmc-demo="BasicUsage">
  <BasicUsage />
</div>
```
In MDX, you can write any Markdown syntax, JSX syntax, or import React components through `import`. If you are not familiar with the use of MDX, please refer to [this document](https://mdxjs.com/). By wrapping the component Demo with `<div data-ddmc-demo="YourDemoName" />`, we will automatically display the source code corresponding to Demo below it:
![](https://p9-ddmcsite.byteimg.com/tos-cn-i-goo7wpa0wc/535269455044439590f4da9940a7af96~tplv-goo7wpa0wc-image.image)
...
Only `interface` or `type` declarations with `@title` will be extracted. You can specify which files to extract the API from in the jsdoc field in ddmc.workspace.jsonc.
...
### Document split
**Version requirement: @ddmc-cli/ddmc >= 2.1.0**
If your component has a large number of demos, you can split them through the built-in component `DdmcMDXPreviewSplit` we provide. The usage is as follows:
```Markdown
// index.mdx
---
title: List
...
![](https://p9-ddmcsite.byteimg.com/tos-cn-i-goo7wpa0wc/8a18ce45b7074998be33860371408e1e~tplv-goo7wpa0wc-image.image)
...
![](https://p9-ddmcsite.byteimg.com/tos-cn-i-goo7wpa0wc/403abe878cff4102acfa0540e06dcc0a~tplv-goo7wpa0wc-image.image)
...
If you are a ByteDance intranet user, use the `ddmc host ddmc.bytedance.net` command to switch the Ddmc domain used in the CLI to the intranet version.
...
The `sync` command requires a line verification of the user identity. Please make sure that you have logged in as a user through the `ddmc login` command before executing this command.
...
`DDMC_CONFIG_X_DDMC_ACCESS_TOKEN=YOUR_ACCESS_TOKEN ddmc sync`
...
![](https://p9-ddmcsite.byteimg.com/tos-cn-i-goo7wpa0wc/a9e4ebdc287c4c24b17a8ccb3f8e9360~tplv-goo7wpa0wc-image.image)
...
<img src="https://p9-ddmcsite.byteimg.com/tos-cn-i-goo7wpa0wc/959331ca87f447589147f4eb4a848634~tplv-goo7wpa0wc-image.image"/>
<img src="https://p9-ddmcsite.byteimg.com/tos-cn-i-goo7wpa0wc/9d12f46733544d9094a3dcd74e68ea6f~tplv-goo7wpa0wc-image.image"/>
</div>

## Tips
### Execute commands on specific components
Ddmc CLI's `start/build/test/sync` commands all support specifying component ID filtering rules to execute commands only on specific components.
