...
Before entering the actual development, we recommend that you read the [Material Development Specification](https://ddmc.design/docs/material/spec). This specification contains some of the best practices and things to avoid in material development that we have summarized.
...
## Development
Through the `ddmc start` command, you can enter the development mode to preview all components in the current workspace. Use `ddmc start ComponentName` to specify the component that needs to start preview.
...
You can configure the entry for component preview documentation and API documentation parsing in the component field in `ddmc.workspace.jsonc`:
```JSON
{
...
###### Specify Ddmc theme package
First of all, it should be clear that Ddmc theme package should be introduced by business projects according to their own needs. The material itself should not directly introduce a theme package into the material's own style, or use it as an NPM dependency. "Using Ddmc theme package" here only refers to introducing the theme package style in the material Demo document to show the performance of this material under a certain theme.
We provide two ways to introduce Ddmc theme package in material preview:
######## Configure component additional style
We provide a simple way to dynamically apply different theme styles to each component, by adding configuration to the component in `ddmc.workspace.jsonc` to provide a list of themes to choose from. When previewing the material, the `.css` file corresponding to the theme will be introduced as an outline style.
...
{
"title": "Linear Theme",
"href": "https://sf-unpkg-src.bytedance.net/@ddmc-design/theme-line@latest/css/ddmc.css",
}
]
}
]
...
If you need more customized operations for the compilation of the theme package, you can install the theme package in the project by yourself, and then import it through the Ddmc Webpack plug-in:

1. Add the theme package in the project development dependencies (devDependencies);

2. Add in the project development dependencies (devDependencies) [DdmcWebpack plugin](https://www.npmjs.com/package/@ddmc-plugins/webpack-react), used for on-demand loading of Ddmc component styles and application of themes;
3. After the dependency is updated, add the relevant configuration of the DdmcWebapck plugin in `ddmc.env.config.js`:
...
If you need to use the GlobalContext above in each component Demo, you can add the configuration field for `previewContextProvider` in `ddmc.workspace.jsonc`:
```JSON
// ddmc.workspace.jsonc
{
"ddmc.aspect/workspace": {
"components": [
{
...
We provide the `window.__registerDdmcPreviewEventListener` method for the component preview page, which is used to listen to events such as dark mode switching and component theme (extraStyles) switching.
```TypeScript
type EventType =
...
// Dark mode switch
| 'switchDarkMode'
// Tab switch event when the component document is split by DdmcMDXPreviewSplit
| 'switchActiveTab'
// Page scrolling
| 'scrollIntoView'
...
(window.__registerDdmcPreviewEventListener as RegisterFn)?.('switchDarkMode', (event) => {
...
![Image](https://p9-ddmcsite.byteimg.com/tos-cn-i-goo7wpa0wc/0e20e7e1222f4e57b55d3eccc936cc35~tplv-goo7wpa0wc-image.image)
#### Style
...
var _button = require('@ddmc-design/web-react/lib/button');
require('@ddmc-design/web-react/lib/button/style/index.js');
...
// Import the Ddmc component style that the current component depends on (if any)
import '@ddmc-design/web-react/es/Button/style';
```
...
Use the `ddmc.workspace.jsonc` file to configure the matching rules for unit test files:
```JSON
{
"ddmc.aspect/workspace": {
"components": [
...
Use the `ddmc test` command to test the components in the workspace. Use `ddmc test ComponentName` to specify the component to be tested.
![](https://p9-ddmcsite.byteimg.com/tos-cn-i-goo7wpa0wc/a2ac18a16b8741e5b9a1ce2a8b5dee19~tplv-goo7wpa0wc-image.image)

#### Pass in Jest CLI parameters
Since the `ddmc test` command is an upper-level encapsulation of the test tool, we avoid letting it directly accept the CLI command parameters of a test tool (such as [Jest CLI](https://jestjs.io/docs/cli)). If you need to pass parameters to Jest CLI, you can pass it through `--rawTesterArgs`:
```Bash
ddmc test --rawTesterArgs="jest -u --silent=false"
```

## Build
The `ddmc build` command can be used to build all components in the workspace. Under React Env, the component build product includes three parts:
```Bash
...
ddmc build ComponentA,ComponentB
...
ddmc build --tasks="TSCompilerESM,TSCompilerCJS"
```

![](https://p9-ddmcsite.byteimg.com/tos-cn-i-goo7wpa0wc/d140aa58f3d04e2aab71c8d38202cc23~tplv-goo7wpa0wc-image.image)
...
![](https://p9-ddmcsite.byteimg.com/tos-cn-i-goo7wpa0wc/434f16a40e6940e788c06f5b9fbc5d5b~tplv-goo7wpa0wc-image.image)
...
/**
* Jest module path (to replace ddmc-cli's internal dependency on Jest)
*/
...
typescript?: {
/**
* Pass in TypeScript module to replace ddmc-cli's internal dependency TypsScript
*/
tsModule?: any;
/**
...
View Details of [TypescriptConfigMutator](https://github.com/ddmc-design/ddmc-cli/blob/next/packages/aspect/src/typescript/typescriptConfigMutator.ts) and [WebpackConfigMutator](https://github.com/ddmc-design/ddmc-cli/blob/next/packages/aspect/src/webpack/webpackConfigMutator.ts).
#### Expand TS configuration
React Env has built-in [default configuration](https://github.com/ddmc-design/ddmc-cli/blob/next/packages/react/src/typescript/tsconfig.json) for TypeScript build, which can be packaged normally even if the `tsconfig.json` component does not exist in the project. But in addition to the `typescript` field in the above `ddmc.env.config.js`, we also allow custom TS build configuration through `tsconfig.json`. The `tsconfig.json` or `tsconfig.build.json` in the root directory of the NPM package will be expanded to the build configuration. At this time, their priorities are from high to low:

1. TS configuration specified by `ddmc.env.config.js`;
...
Customize the Jest configuration file path by configuring the `jest.jestConfigPath` field of `ddmc.env.config.js`.
...
```JavaScript
// jest.config.js
const defaultConfig = require(require.resolve('@ddmc-cli/react/dist/jest/jest.cjs.config.js'));
...
We provide the `postBuild` option for `ddmc.service/compiler`, through which you can customize the subsequent tasks after the build is completed.
```JSON
// ddmc.workspace.jsonc
{
// This configuration item is used to specify the default behavior of TSCompilerESM / TSCompilerCJS
"ddmc.service/compiler": {
// Specify the subsequent tasks after the component is built, receive a string, which is the path of the script file relative to the root directory of the Ddmc workspace
"postBuild": './postBuild.js'
}
}
```

###### Automatically inject Ddmc style dependencies into the Less entry of the product
The following task will automatically inject the dependent Ddmc component style dependencies into its `/{es,lib}/ComponentName/style/index.js` after the component is built:
```JavaScript
...
```JavaScript
// /es/Component/style/index.js
import '@ddmc-design/web-react/es/Radio/style/index.js';
import './index.less';
//## sourceMappingURL=index.js.map
```
