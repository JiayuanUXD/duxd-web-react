...
Ddmc CLI 引入了组件运行环境（Env）的概念

`````

我们为 Ddmc CLI 引入了组件运行环境（Env）的概念，旨在为基于不同框架（React / Vue / Angular）、不同执行环境（Browser / Node）的组件提供统一的上层交互和开发体验。所以对开发环境的配置拓展，均为对 Env 的拓展。
...
## 遵守规范
在进入实际开发之前，我们建议你阅读[物料开发规范](https://ddmc.design/docs/material/spec)。此规范包含了我们总结的一些物料开发中的最佳实践和应避免的事项。
你不需要完全遵守规范中所提到的内容，但同一团队内的所有物料都应尽量保证使用同一开发规范。
## 开发
通过 `ddmc start` 命令，可以进入开发模式以预览当前工作区内的所有组件。通过 `ddmc start ComponentName` 来指定所需要启动预览的组件。
...
你可以在 `ddmc.workspace.jsonc` 中的组件字段中配置组件预览文档和 API 文档解析的入口：
```JSON
{
...
###### 指定 Ddmc 主题包
首先需要明确的是，Ddmc 主题包应该由业务项目根据自身需求自行引入。物料本身**不应该**直接将某一主题包引入物料自身样式，或者将其作为 NPM 依赖。此处所说的 “使用 Ddmc 主题包”仅仅指在物料 Demo 文档中引入主题包样式，以展示此物料在某一主题下的表现。
我们提供了两种形式来在物料预览中引入 Ddmc 主题包：
######## 配置组件额外样式
我们提供了一种简便的方式来为每一组件动态应用不同的主题样式，通过在 `ddmc.workspace.jsonc` 中为组件增添配置来提供可供选择的主题列表。物料预览时，主题对应的 `.css` 文件将以外联样式的形式引入。
...
######## 通过 Webpack 插件引入
如果你对主题包的编译需要更多自定义操作，那么可以自行在项目中安装主题包，然后通过 Ddmc Webpack 插件将其引入：

1. 在项目开发依赖（devDependencies）中添加主题包；
2. 在项目开发依赖（devDependencies）中添加 [DdmcWebpack 插件](https://www.npmjs.com/package/@ddmc-plugins/webpack-react)，用于 Ddmc 组件样式按需加载和应用主题；
3. 依赖更新后，在 `ddmc.env.config.js` 中添加 DdmcWebapck 插件的相关配置：
...
如果你需要在每一个组件 Demo 中使用上边的 GlobalContext，可以在 `ddmc.workspace.jsonc` 中添加关于 `previewContextProvider` 的配置字段：
```JSON
...
###### 监听文档页面事件
我们为组件预览页面提供了 `window.__registerDdmcPreviewEventListener` 方法，用于监听如暗色模式切换、组件主题（extraStyles）切换等事件。
```TypeScript
type EventType =
  ...
  // 暗色模式切换
  | 'switchDarkMode'
  // 组件文档由 DdmcMDXPreviewSplit 拆分时，Tab 切换的事件
  | 'switchActiveTab'
  // 页面滚动
  | 'scrollIntoView'
  // 更新右侧锚点相对于页面顶部的偏移量
...
为保证组件使用文档简洁和方便工具自动提取生成版本日志文档，我们支持为组件添加除预览文档之外的其它文档。通过 `ddmc.workspace.jsonc` 中的组件配置字段来添加额外的文档入口（仅支持 `.md` 文件）：
```JSON
{
  "ddmc.aspect/workspace": {
    "components": [
...
![图片](https://p9-ddmcsite.byteimg.com/tos-cn-i-goo7wpa0wc/0e20e7e1222f4e57b55d3eccc936cc35~tplv-goo7wpa0wc-image.image)
#### 样式
...
通过 `ddmc.workspace.jsonc` 文件配置单元测试文件的匹配规则：
```JSON
{
  "ddmc.aspect/workspace": {
    "components": [
...
通过 `ddmc test` 命令来对工作区内的组件进行测试。通过 `ddmc test ComponentName` 来指定需要测试的组件。
![](https://p9-ddmcsite.byteimg.com/tos-cn-i-goo7wpa0wc/a2ac18a16b8741e5b9a1ce2a8b5dee19~tplv-goo7wpa0wc-image.image)

#### 传入 Jest CLI 参数
由于 `ddmc test` 命令是对于测试工具的上层封装，我们避免了让它直接接受某一测试工具的 CLI 命令参数（例如[ Jest CLI](https://jestjs.io/docs/cli)）。如果你需要为 Jest CLI 传入参数，可以通过 `--rawTesterArgs` 来传入：
```Bash
ddmc test --rawTesterArgs="jest -u --silent=false"
```

## 构建
通过 `ddmc build` 命令，可以对工作区内的所有组件进行构建。React Env 下，组件的构建产物包括三部分：
```Bash
...
在完成 NPM 发布之后，通过 `ddmc sync` 命令将最新的组件文档同步至物料平台。
## 可拓展配置
#### 配置文件详解
类似于 Workspace 配置，我们会在项目根目录查找名为 `ddmc.env.config.js` 的文件，此文件即为所有组件 Env 的配置入口。它需要暴露一个返回配置对象的函数，函数可接收一个用于区分不同 Env 的参数：
```JavaScript
// ddmc.env.config.js
...
type DdmcReactEnvConfig = {
  ...
  jest?: {
    ...
    /**
     * Jest 模块路径（用以替代 ddmc-cli 内部依赖的 Jest）
     */
  ...
  typescript?: {
    /**
     * 传入 TypeScript 模块以替代 ddmc-cli 内部依赖的 TypsScript
     */
    tsModule?: any;
    /**
     * 拓展物料产物构建时的配置
...
查看 [TypescriptConfigMutator](https://github.com/ddmc-design/ddmc-cli/blob/next/packages/aspect/src/typescript/typescriptConfigMutator.ts) 和 [WebpackConfigMutator](https://github.com/ddmc-design/ddmc-cli/blob/next/packages/aspect/src/webpack/webpackConfigMutator.ts) 的详细内容。
####  拓展 TS 配置
React Env 已经内置了 TypeScript 构建的[默认配置](https://github.com/ddmc-design/ddmc-cli/blob/next/packages/react/src/typescript/tsconfig.json)，即使项目中不存在 `tsconfig.json` 组件也能够正常打包。但除了通过上述 `ddmc.env.config.js` 中 `typescript` 字段，我们也允许通过 `tsconfig.json` 来自定义 TS 构建配置。NPM 包根目录的 `tsconfig.json` 或者 `tsconfig.build.json` 都会被拓展到构建配置中，此时它们之间的优先级从高至低为：

1. `ddmc.env.config.js` 指定的 TS 配置；
2. NPM 包根目录 `tsconfig.json` / `tsconfig.build.json` 指定的配置；
3. React Env 中的默认 TS 配置。

#### 拓展 Jest 配置
通过配置 `ddmc.env.config.js` 的 `jest.jestConfigPath` 字段来自定义 Jest 配置文件路径。
```JavaScript
// ddmc.env.config.js
...
#### 拓展构建完成的后续任务（>= 2.3.1）
我们为 `ddmc.service/compiler` 提供了 `postBuild` 选项，通过此配置可以自定义构建完后成的后续任务。
```JSON
// ddmc.workspace.jsonc
{
  // 此配置项用于指定 TSCompilerESM / TSCompilerCJS 的默认行为
  "ddmc.service/compiler": {
    // 指定组件构建完成之后的后续任务，接收一个字符串，为脚本文件相对于 Ddmc 工作区根目录的路径
    "postBuild": './postBuild.js'
  }
}
```

###### 自动在产物的 Less 入口注入对 Ddmc 的样式依赖
以下任务将在组件构建完成之后，在其 `/{es,lib}/ComponentName/style/index.js` 中自动注入所依赖的 Ddmc 组件样式依赖：
```JavaScript
