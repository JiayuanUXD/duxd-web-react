...
* 单包组件：以 `@ddmc-materials/select-with-check-all` 为例，其包内只包含了 `SelectWithCheckAll` 一个组件，此组件对应的物料 ID 为 `@ddmc-materials/select-with-check-all/SelectWithCheckAll`；
* 组件库：以 `@ddmc-design/web-react` 为例，其包内包含了多个组件，其组件对应的物料 ID 为 `@ddmc-design/web-react/ComponentName`。
...
## 新增物料
如果你之前全局安装过 ddmc-cli 1.x 的 NPM 包，运行 `ddmc xxx` 命令时，Node 可能会命中 1.x 的 CLI 命令。此种情况下，你可以使用 `npm uninstall ddmc-cli -g` 卸载 1.x CLI，或使用 `npx ddmc xxx` 指定 Node 优先从当前项目的 node_modules 中查找可执行命令。
...
在 MDX 中，你可以书写任意 Markdown 语法、JSX 语法，或者通过 `import` 引入 React 组件。如果你还不熟悉 MDX 的使用，请参阅[此文档](https://mdxjs.com/)。通过以 `<div data-ddmc-demo="YourDemoName" />` 包裹组件 Demo，我们将会自动将 Demo 对应的源码展示在其下方：
...
只有带有 `@title` 的 `interface` 或者 `type` 声明都会被提取，可以在 ddmc.workspace.jsonc 里的 jsdoc 字段指定从哪些文件进行 API 提取。
属性成员有以下可用的描述标签：
...
### 文档拆分
**版本要求：@ddmc-cli/ddmc >= 2.1.0**
如果你的组件存在大量的 Demo，可以通过我们提供的内置组件 `DdmcMDXPreviewSplit` 对它们进行拆分。使用方式如下：
```Markdown
// index.mdx
---
title: List
...
### 同步至物料平台
如果你是 ByteDance 内网用户，请使用 `ddmc host ddmc.bytedance.net` 命令将 CLI 中所使用的 Ddmc 域名切换至内网版本。
...
`sync` 命令需要对用户身份进A行校验，在执行此命令前请确认你已通过 `ddmc login` 命令进行过用户登录。
...
## Tips
### 对特定组件执行命令
Ddmc CLI 的 `start/build/test/sync` 命令均支持指定组件 ID 过滤规则来仅对特定的组件执行命令。
