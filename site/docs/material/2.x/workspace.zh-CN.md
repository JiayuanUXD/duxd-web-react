...
## 配置
默认地，我们在项目根目录查找名为 `ddmc.workspace.jsonc` 或 `ddmc.workspace.js` 的文件以应用工作区配置。绝大多数 CLI 相关的命令都只能在 Ddmc 物料工作区内执行，否则将会得到以下错误：
```Bash
...
## 初始化工作区
我们提供了名为 `@ddmc-cli/generator` 的组件用来初始化工作区，执行以下命令：
```Bash
## 创建一个标准工作区，它是单包多组件形式
...
### 配置组件创建时的默认行为
在 `ddmc.workspace.jsonc` 中，通过以下字段配置新增组件时的一些默认行为：
```JSON
{
  "ddmc.service/generator": {
    // 新增组件的默认目录，等同 ddmc create --path="workspace-path/defaultPath"
    "defaultPath": "src",
    // 新增组件时使用的默认模板
...
### 自定义组件模板
你可以将本地目录指定为 `ddmc create` 时的组件模板。以下文件会被视为特殊文件，它们将不会被直接拷贝至目标目录（除此之外的所有文件，都将直接被拷贝至目标目录）：

* `__ddmc.tpl.desc.json` 模板描述文件，它所接收的字段类型如下：
...
interface TemplateDesc {
  ...
  name: string
  // 仅组件模板需要
  // 可接受 ddmc.workspace.jsonc 组件配置中相同的 entries 字段，用于在组件创建完成后自动填充 Workspace 配置
  entries?: {}
}
```


* `__ddmc.dir.desc.js` 文件目录的描述文件，它需要暴露一个CommonJS 模块：
...
可参考 Ddmc CLI [内置模板](https://github.com/ddmc-design/ddmc-cli/tree/next/packages/generator/src/templates)。需要注意，此示例中的模板文件经过了 Babel 编译， `*.tpl.js` 和 `__ddmc.dir.desc.js` 必须遵循 CommonJS 规范。
