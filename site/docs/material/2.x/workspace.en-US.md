...
## Configuration
By default, we look for a file named `ddmc.workspace.jsonc` or `ddmc.workspace.js` in the project root directory to apply the workspace configuration. Most CLI-related commands can only be executed in the Ddmc material workspace, otherwise you will get the following error:
```Bash
workspace not found, please run ddmc command in a ddmc workspace
```
...
We provide a component named `@ddmc-cli/generator` to initialize the workspace. Execute the following command:
```Bash
## Create a standard workspace, which is a single package and multiple components
npx @ddmc-cli/generator new your-workspace-name

## Create a Monorepo workspace
npx @ddmc-cli/generator new your-workspace-name --templateArgs="--monorepo"
...
npx ddmc create your-package-name --template=react-package --path=packages
```

`@ddmc-cli/generator` Command Details:
```Plain Text
ddmc-generate new <name>

create an empty ddmc material workspace
...
```Bash
npx ddmc create Button

## --path specifies the parent directory for creating components
npx ddmc create Button --path="packages/my-sub-package/src"
```

### Configure default behaviors when creating components
In `ddmc.workspace.jsonc`, use the following fields to configure some default behaviors when adding new components:
```JSON
{
"ddmc.service/generator": {
// Default directory for adding new components, equivalent to ddmc create --path="workspace-path/defaultPath"
"defaultPath": "src",
...
You can specify a local directory as a component template when `ddmc create`. The following files will be treated as special files and will not be copied directly to the target directory (all other files will be copied directly to the target directory):

* `__ddmc.tpl.desc.json` template description file, the field types it receives are as follows:
...
// Accepts the same entries field in ddmc.workspace.jsonc component configuration, used to automatically fill in the Workspace configuration after the component is created
entries?: {}
}
```

* `__ddmc.dir.desc.js` The description file of the file directory needs to expose a CommonJS module:
...
Please refer to the Ddmc CLI [built-in templates](https://github.com/ddmc-design/ddmc-cli/tree/next/packages/generator/src/templates). Note that the template files in this example have been compiled by Babel, and `*.tpl.js` and `__ddmc.dir.desc.js` must follow the CommonJS specification.
