...
As of version `1.26.0`, `@ddmc-design/ddmc-cli` has been renamed to `ddmc-cli`. We apologize for the inconvenience caused by the package name change, please use the following commands to uninstall the old package and install the latest package:

```bash
# Uninstall old version CLI
npm uninstall @ddmc-design/ddmc-cli -g
...
* As of `1.25.15`, `@ddmc-design/ddmc-scripts` has been changed to `ddmc-scripts`.
* As of `1.9.3`, `@ddmc-design/ddmc-doc-site` has been changed to `ddmc-material-doc-site`.

## How to create a material team?

The material platform does not allow users to create teams by themselves, please go to [this page](https://ddmc.design/material/createGroup/) to apply. After the creation is complete, the team owner can manage the team members and information by themselves.
...
![](https://p1-ddmc.byteimg.com/tos-cn-i-uwbnlip3yd/be6d4f61e5423b9be492206d88bdb139.png~tplv-uwbnlip3yd-webp.webp)

## Which project template to choose?

Ddmc officially provides a variety of project templates for you to choose:
...
-Quickly experience the development process and platform capabilities of Ddmc materials: business component templates;
...
It can be used, but you need to ensure that ddmc-scripts >= 1.20.5.

## After the material development is completed, how to preview its display effect on the material platform?

You can directly preview the local materials through the `ddmc preview` command provided by ddmc-cli or the `npm run preview` script provided in the project.

## How to support the business side to load Ddmc component styles on demand?
...
In order to support users to load the Ddmc component styles on which materials depend on demand, the following steps are required:

-Material provider: Declare the dependent Ddmc component style in the component `/style/index.ts`

```typescript
// src/style/index.ts
import'./index.less';

// If the material uses Ddmc components, declare its dependencies
...
![](https://p1-ddmc.byteimg.com/tos-cn-i-uwbnlip3yd/screenshot-20220408-145533.png~tplv-uwbnlip3yd-webp.webp)

On the group page, click the "⚙" button to configure the team settings (only available for team administrators), search for the desired related theme and submit it. After completing the configuration, the platform will preferentially load the associated theme package instead of Ddmc's default style when previewing the material.
...
**Note: `ddmc-material-doc-site >= 1.10.0` is required and the NPM package corresponding to the theme needs to be installed locally, otherwise an error will be reported due to missing modules in Dev mode.**
...
![](https://p1-ddmc.byteimg.com/tos-cn-i-uwbnlip3yd/screenshot-20220408-153706.png~tplv-uwbnlip3yd-webp.webp)
...
module.exports = {
  ...
  site: {
    ...
    ddmcDesignLabTheme: '@ddmc-design/theme-volcengine-ui'
  },
};
````
...
When creating a material project in `ddmc init`, we provide a series of material keywords to choose from. These keywords are convenient for users to quickly filter materials. Team managers can customize the list of alternative keywords by configuring:

![](http://p1-ddmc.byteimg.com/tos-cn-i-uwbnlip3yd/screenshot-20220420-150133.png~tplv-uwbnlip3yd-webp.webp)

Once configured, the team material list will be filtered using the configured keyword list. Ddmc CLI can also link specific teams via `ddmc group --link`:

**Version requirements: ddmc-cli >= 1.27.0**
...
* `ddmc init` will use the keyword list configured by the team as an alternative when creating a project;
* When `ddmc sync` publishes materials, the materials will be released to the associated team first.
...
// to
import {Button} from'@ddmc-design/web-react/lib';
```

## Dark theme

The material supports the dark theme without additional configuration, just use Ddmc's built-in color variables to define the color in the component development. Reference [DdmcDesign | Dark Mode](https://ddmc.design/react/docs/dark)
