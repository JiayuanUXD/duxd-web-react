...
在执行 `ddmc init` 时，Git 根目录中将会创建 `ddmc.config.js` 用于配置 `ddmc` 命令的行为。
...
详细配置参数类型和默认配置可参考 [ddmc-scripts 源码](https://github.com/ddmc-design/ddmc-cli/tree/main/packages/ddmc-scripts/src/config) 。

Ddmc 官方模板所创建的项目使用 `ddmc-scripts` 来进行项目的测试和打包，你可以通过配置文件修改其默认配置。在项目根目录的 `.config` 文件夹内，我们提供了扩展配置的入口：
...
- `/docs` 组件的 Props 文档和 Demo 用法文档，有 `ddmc-scripts docgen` 命令构建；
...
**版本要求 `ddmc-scripts >= 1.25.7`**

`ddmc-scripts` 的配置项由 TypeScript 书写，你可以通过 IDE 和 JsDoc 的配合来实现智能提示：
...
// webpack.config.js
const path = require('path');
// webpack-merge 已经被 ddmc-scripts 依赖，你无需手动安装它
...
我们提供了 `ddmc-scripts show:config <configType>` 命令快速查看当前生效的相关配置。
...
For more details, view [/Users/ddmc/workspace/ddmc-cli/packages/ddmc-scripts/config/style.config]
```
