...
When executing `ddmc init`, `ddmc.config.js` will be created in the Git root directory to configure the behavior of the `ddmc` command.
...
For detailed configuration parameter types and default configuration, please refer to [ddmc-scripts source code](https://github.com/ddmc-design/ddmc-cli/tree/main/packages/ddmc-scripts/src/config).

The project created by the official Ddmc template uses `ddmc-scripts` to test and package the project, and you can modify its default configuration through the configuration file. In the `.config` folder of the project root directory, we provide an entry for the extended configuration:
...
- Props documentation and Demo usage documentation of the `/docs` component, built with the `ddmc-scripts docgen` command;
...
**Version requirement `ddmc-scripts >= 1.25.7`**

Since `ddmc-scripts` ships with TypeScript typings, you can leverage your IDE's intellisense with jsdoc type hints:

```js
// docgen.config.js

/**
  * @param config {import('ddmc-scripts').DocgenConfig}
  */
module.exports = (config) => {};
```

![](https://p1-ddmc.byteimg.com/tos-cn-i-uwbnlip3yd/0cecc7d5998e81f8841460f205dffd5c.png~tplv-uwbnlip3yd-webp.webp)
...
// webpack.config.js
const path = require('path');
// webpack-merge is already dependent on ddmc-scripts, you don’t need to install it manually
...
exports.node = (config) => {
  ...
  config.moduleNameMapper = {
    '^@ddmc-design/hello-ddmc/(.+)$':'<rootDir>/$1',
    '^@ddmc-design/hello-ddmc$':'<rootDir>',
  };
};
...
We provide the `ddmc-scripts show:config <configType>` command to quickly view the current effective configuration.

```bash

$ ddmc-scripts show:config --help

Usage: ddmc-scripts show:config <configType>

Show your current config for ddmc-scripts. Valid type: babel|style|webpack.component|webpack.site|webpack.icon|jest|docgen
