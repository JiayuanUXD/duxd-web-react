...
Ddmc officially provides a variety of material templates. When they cannot meet your needs, you can develop your own material templates. It is recommended to modify based on Ddmc's official material item, and then convert it into a material template.

## **Convert the project into a template**

Assuming that you already have a secondary-developed Ddmc material project, you can quickly convert it into a template through the `ddmc template --create` command.

```
$ ddmc template --create
...
? Please enter the path of the existing project as a template ./hello-ddmc
...
After the `ddmc init` initialization project is completed, it will try to execute the custom hook function of the template. Here you can customize the help information, perform project construction, etc. Add `hook/after-init.js` to the root directory of the project. Below is an example of a hook function:
...
ddmc init hello-ddmc --template file:../path/to/your/template
```

## **Publish and use templates**

After the template is developed and debugged correctly, it can be used after it is published to NPM. You can use this template to initialize the material project by `ddmc init hello-ddmc --template yourTemplatePackageName`.
