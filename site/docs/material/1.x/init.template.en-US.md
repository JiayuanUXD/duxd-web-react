...
Learn about various material project templates officially provided by Ddmc.
`````
...
Currently, Ddmc provides the following templates:
...
- Component library - used to create a business component library project with a structure similar to [Ddmc Component Library](https://github.com/ddmc-design/ddmc-design) **(recommended)**;
- Monorepo - used to create Lerna-managed Monorepo projects **(recommended)**;

To use the above template, just select the corresponding category during `ddmc init`.

### Technology Stack

In the project created by the official Ddmc template, the following technology stacks are used:
...
```
├── .config // ddmc-scripts configuration items, including build, test, and document generation. For details, see the next chapter [Custom Configuration]
│ ├── babel.config.js
│ ├── docgen.config.js
│ ├── jest.config.js
│ ├── style.config.js
...
│ └── preview.js
│
├── ddmc.config.js // ddmc-cli configuration item
│
...
Using the Monorepo template to manage all the materials of the team is the best practice recommended by Ddmc. Below we will introduce this template in detail. The Monorepo project created by `ddmc init` is a skeleton project without any Package. The directory structure and detailed description are as follows:
...
│
├── ddmc.config.js // ddmc-cli configuration item
├── ddmc.scripts.config.js // ddmc-scripts configuration items common to all packages
├── lerna.json
├── package.json
...
```
├── .config // Expand ddmc-scripts configuration items, including build, test, and document generation. For details, see the next chapter [Custom Configuration]
│ ├── babel.config.js
│ ├── docgen.config.js
│ ├── jest.config.js
│ ├── style.config.js
...
- Use the `ddmc generate` and `ddmc sync` commands to generate and synchronize all material meta information. If you only need to operate on a certain material, the above two commands both support the `--from-current-package` parameter;

## Custom template

ddmc-cli allows you to create a project by specifying a template (local/online), using `--template` to point to a local path or an npm package, which will be very useful when you develop custom templates.

```bash
ddmc init <projectName> --template [templatePackageName|file:templatePath]

# e.g.
ddmc init my-site --template @ddmc-materials/template-core

ddmc init my-site --template file:../path/to/your/template/acro-template-site
```
