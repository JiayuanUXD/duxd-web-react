...
Use the `ddmc init ddmc-team-site --pure` command to initialize the site directory in the project. (Ddmc CLI version >= 1.19.0)

The command needs to fill in two parameters:

- The package name of the template: @ddmc-materials/template-team-site
- The package name of the site project: it's up to you.

![](https://p1-ddmc.byteimg.com/tos-cn-i-uwbnlip3yd/94a1d3c76247c37f82fb4f5d52933740.png~tplv-uwbnlip3yd-webp.webp)
...
**Version requirement `ddmc-material-doc-site >= 1.4.0`**

Since `ddmc-material-doc-site` ships with TypeScript typings, you can leverage your IDE's intellisense with jsdoc type hints:

```js
// .config/main.js

/**
 * @type {import('ddmc-material-doc-site').MainConfig}
 */
module.exports = {... };
```

The complete `MainConfig` configuration field declaration please [go here](https://github.com/ddmc-design/ddmc-cli/blob/main/packages/ddmc-material-doc-site/src/interface.ts#L55).

```js
// .config/webpack.config.js

/**
 * @param config {import('ddmc-material-doc-site').WebpackConfig}
 */
module.exports = (config) => {};
```

![](https://p1-ddmc.byteimg.com/tos-cn-i-uwbnlip3yd/da38d96ab9856876f34f90e0fa05f514.png~tplv-uwbnlip3yd-webp.webp)
...
Refer to the following file content. If your project is created based on the Ddmc "Component Library" or "Monorepo" material template, select the correct `build.globs.component` value.
...
module.exports = {
  ...
  build: {
    ...
    globs: {
      // Can be used for configuration of Ddmc Monorepo template
      component: {
...
![](https://p1-ddmc.byteimg.com/tos-cn-i-uwbnlip3yd/585db8128cbcf0a12ab0ab6f70051450.png~tplv-uwbnlip3yd-webp.webp)
...
module.exports = (config) => {
  config.resolve.alias = {
    '@ddmc-design/rc-xxx': path.resolve('../components'),
  };
};
```
...
The site dependency package `@ddmc-desgin/ddmc-doc-site` provides the `ddmc-doc-site dev` command to start the development mode of the team site locally as an alternate option for Storybook. In this way, even if you do not create a project using the official material template provided by Ddmc, you can easily develop materials.
...
```jsx
import {Button} from'@ddmc-design/my-material';
...
// .config/webpack.config.js
module.exports = (config) => {
   config.resolve.alias['@ddmc-design/my-material'] ='/project-root/packages/my-material/src';
};
```

![](https://p1-ddmc.byteimg.com/tos-cn-i-uwbnlip3yd/c6001648f6b03f932f8b99e5505dbc1e.png~tplv-uwbnlip3yd-webp.webp)
...
![](https://p1-ddmc.byteimg.com/tos-cn-i-uwbnlip3yd/386069293bf5b040ee09c6c233be765d.png~tplv-uwbnlip3yd-webp.webp)
...
![](https://p1-ddmc.byteimg.com/tos-cn-i-uwbnlip3yd/d14e7b6b380cee60d66e7180d05420d5.png~tplv-uwbnlip3yd-webp.webp)
...
- Side menu behavior
- Custom page module
- [DdmcDesignLab](https://ddmc.design/themes) theme package association
- [DdmcIconBox](https://ddmc.design/iconbox/libs) icon library association

For details of configuration fields, please [go here](https://github.com/ddmc-design/ddmc-cli/blob/main/packages/ddmc-material-doc-site/src/interface.ts#L93) , refer to `MainConfig.site` field type.
...
Expose a module with a specific name in `customModule.tsx` and the site page will render it into the page. Currently customizable modules include `Navbar | Footer | Menu | Sider | DocHeader | Affix`. [Demo](https://github.com/ddmc-design/official-material-react/blob/main/team-site/fixtures/customModule.tsx)

![](https://p1-ddmc.byteimg.com/tos-cn-i-uwbnlip3yd/screenshot-20221010-153712.png~tplv-uwbnlip3yd-webp.webp)

## Using the Ddmc Design Lab theme
...
## Specify Ddmc version

By default, we have already externalized `@ddmc-design/web-react` when building the site, so the packaged product does not contain the Ddmc component library code and styles, they will be injected globally by the site page. You can configure the Ddmc component library version it uses in [Material Platform-Team Page-Team Site].

![](https://p1-ddmc.byteimg.com/tos-cn-i-uwbnlip3yd/cc7585da8cb507694e28f1d30d7d557a.png~tplv-uwbnlip3yd-webp.webp)

> When you use the `ddmc preview` command to preview the site locally, you can specify the Ddmc component library version by adding a Query parameter of `ddmcVersion=2.12.0` to the URL.

By modifying the webpack configuration, you can avoid externalizing `@ddmc-design/web-react`, but we generally do not recommend this approach (the materials should be available in all Ddmc versions as much as possible).If you do not need to use the globally injected Ddmc component library, you can choose "Do not inject Ddmc".

## Transformation of own projects

The Ddmc document site has been decoupled from the project structure to the greatest extent. Theoretically, the document site can be initialized from any existing project. Follow the steps below:
...
import {ComponentOne} from'@ddmc-design/rc-xxx';
...
**Version requirements `ddmc-material-doc-site >= 1.12.0`**
...
![](https://p1-ddmc.byteimg.com/tos-cn-i-uwbnlip3yd/isolate.png~tplv-uwbnlip3yd-webp.webp)

### Import via React components

We provide a material called `@ddmc-materials/material-site-viewer` for previewing the team site, which you can use in your own projects. Introducing a site for rendering in the form of a component has the following side effects:

- Need to inject React/ReactDOM/ddmc/ddmcicon in the global scope;
- The component styles (or theme package styles) of `@ddmc-design/web-react` need to be imported in full;
...
import * as ddmc from '@ddmc-design/web-react';
import * as ddmcicon from '@ddmc-design/web-react/icon';

import { createBrowserHistory } from 'history';
import { Router } from 'react-router-dom';
import SiteViewer from '@ddmc-materials/material-site-viewer';

// Make sure the ddmc style is introduced globally in the current project
// If the Design Lab theme is used, import the css file of the theme package
import '@ddmc-design/web-react/dist/css/ddmc.min.css';

// Since the site product file has removed React/ReactDOM/Ddmc, it needs to be exposed in the global scope
  ...
  return (
    <Router history={createBrowserHistory({ basename: '' })}>
      <SiteViewer
        ...
        siteFileUrl={{
          js: 'https://unpkg.com/@ddmc-materials/material-team-site@latest/dist/ddmcSite.zh-CN.js',
          css: 'https://unpkg.com/@ddmc-materials/material-team-site@latest/dist/ddmcSite.css',
        }}
