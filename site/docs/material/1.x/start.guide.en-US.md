...
Quickly understand the concept of Ddmc materials and create your first material.
`````

*Auto translate by google.*

[Ddmc Material Platform](https://ddmc.design/material) serves the front-end technical team and is committed to enhancing team collaboration, promoting resource sharing, and improving development efficiency. Based on the technology precipitation from the DdmcDesign component library, we provide a simple and easy-to-learn material development program.
...
In Ddmc's material system, we subdivide materials into three categories: components, blocks, and pages.

### Components

The concept of components is the closest to the basic components provided by Ddmc. They are the basic elements of the page, but may be coupled with business logic. The usage is consistent with the component library, and components are referenced by introducing NPM dependencies.

![](https://p1-ddmc.byteimg.com/tos-cn-i-uwbnlip3yd/5ee537a3b3da4d336517f0c81e1ec3ca.png~tplv-uwbnlip3yd-webp.webp)
...
![](https://p1-ddmc.byteimg.com/tos-cn-i-uwbnlip3yd/fc4eea64d19e85e4f1135c95e76e3cde.png~tplv-uwbnlip3yd-webp.webp)

### Page

The page is its literal meaning, and its usage is similar to the block. A typical example is the page provided by [DdmcDesign Pro](https://ddmc.design/pro/).

![](https://p1-ddmc.byteimg.com/tos-cn-i-uwbnlip3yd/b4e39a05533707af57ce5a083d93ad3f.png~tplv-uwbnlip3yd-webp.webp)

## Ddmc CLI

In order to facilitate developers to quickly build material projects and manage materials. Ddmc provides a Node-based scaffolding tool ddmc-cli, which roughly includes the following functions:

- Create an DdmcDesign-based material project based on the template;
- Publish, manage and use materials;
- Use material blocks or pages in the project;

Before starting further process experience, you need to install Ddmc CLI globally via `npm i ddmc-cli -g`.

## New Project

Through Ddmc CLI, we can quickly create a new material project based on DdmcDesign. Use `ddmc init yourProjectName` to initialize the project:

![](https://p1-ddmc.byteimg.com/tos-cn-i-uwbnlip3yd/cceb75d805f175694a3e907c490e5e84.png~tplv-uwbnlip3yd-webp.webp)
...
- DdmcDesign Pro project: Create a front-end engineering project based on DdmcDesign Pro.
...
![](https://p1-ddmc.byteimg.com/tos-cn-i-uwbnlip3yd/d1da4e1f19ba518ce731d532a8e89406.png~tplv-uwbnlip3yd-webp.webp)

## Add material

According to the instructions of Ddmc CLI, run `yarn add:package myFirstMaterial` to add our first material. Here we need to answer a few questions about material information.

![](https://p1-ddmc.byteimg.com/tos-cn-i-uwbnlip3yd/d5c36e42dd2b9b60cee09efc4bf775d6.png~tplv-uwbnlip3yd-webp.webp)

## Development materials

According to the instructions of Ddmc CLI, run `yarn dev` to develop preview materials.

![](https://p1-ddmc.byteimg.com/tos-cn-i-uwbnlip3yd/c31a051d8426cf0566bea7474547debb.png~tplv-uwbnlip3yd-webp.webp)
...
- Try to use the CSS variables provided by Ddmc to define colors, so that your materials can seamlessly support [Dark Mode](https://ddmc.design/react/docs/dark).
...
- Demo does not support reference to other dependency packages related to React and Ddmc component library by default. If you need to import it, please refer to [Demo dependency](/docs/material/build#demo-dependency).
...
You need to pay attention to `TEMPLATE.md` and `demo`. In projects using TypeScript, ddmc-scripts can quickly generate component interface documentation by extracting the content of comments. `TEMPLATE.md` is a template generated by the document. Its contents are as follows:
...
import TooltipButton from'@ddmc-design/rc-xxx';
...
In the project created by the Ddmc official material template, you don't have to bother to deal with document generation, we will generate a new document before `npm publish` and upload it to NPM.
...
- Modify the `docgen` command in the block package.json to `ddmc-scripts docgen`;
- Make sure ddmc-scripts >= 1.20.4;
...
- After the local development is completed, you can use `yarn prepublishOnly && ddmc preview` to preview the actual display effect of the material platform.

## Preview material

In order to ensure that the developed materials can be previewed normally on the materials platform, we provide the `ddmc preview` command to preview local materials. Before previewing, ensure that the material has been constructed and generated documents, you can quickly achieve it through the following commands:
...
After the NPM is released, we need to synchronize the material information to the material platform. If you are a member of a team and need to publish all materials in the project under the team, you can modify the default `group` field in the `ddmc.config.js` of the project root directory to be your team ID. In this way, all new materials will be released to your team by default when they are released.
...
module.exports = {
  ...
  // e.g. ['packages/*']
  packages: ['packages/*'],
  // command you want to replace'ddmc subCommand'
  // e.g. publish:'lerna publish'
  alias: {
    publish:'lerna publish',
  },
  // initial meta for'ddmc generate'
  initialMeta: {
    // Modify this to be your team ID
    group: 1,
  },
  // path of ddmc block insertion, relative to /src ('myPath' will be resolved as'/src/myPath')
...
ddmc sync --from-current-package
...
The team site is a more centralized display of team materials, and its format is based on the official Ddmc component documentation page. Before creating a team site, you need to go to [this page](https://ddmc.design/material/createGroup/) to apply for creating your own material team.

![](https://p1-ddmc.byteimg.com/tos-cn-i-uwbnlip3yd/6ee1b1c7df7c084da691b5e8797e7ae9.png~tplv-uwbnlip3yd-webp.webp)
