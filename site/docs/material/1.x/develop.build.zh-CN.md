...
ddmc-scripts 的构建命令内置了组件 JS、图标、样式的构建功能。通常情况下，你只需要通过以下命令来构建所有：
...
1.  新建 `/src/demo/ddmcDemoVendor.js` 文件，将所需要用到的依赖从本文件中导出。
...
2.  在 Demo 文件中，从 `ddmcDemoVendor.js` 中引入此依赖。
...
3.  确保 ddmc-scripts 版本 >= 1.17.1，并且 Webpack 构建的配置中包含了 ddmcDemoVendor 入口。
...
为了方便开发时快速查看代码修改效果，ddmc-scripts 提供了以下命令监听代码变动以自动构建：
