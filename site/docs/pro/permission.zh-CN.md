...
权限控制是中后台场景非常常见的基础功能，在 `v2.1.0` 将权限控制功能集成至 Ddmc Design Pro
`````
## 适用场景
...
Ddmc Design Pro 在尽力满足以上场景。具体效果可前往 [Ddmc Design Pro](https://react-pro.ddmc.design/) 体验。

![权限控制](http://p1-ddmc.byteimg.com/tos-cn-i-uwbnlip3yd/20230414-182335.gif~tplv-uwbnlip3yd-3.awebp)
...
在权限判断上，Ddmc Design Pro 不仅支持直接的 `resource` 字符串判断，还支持正则匹配的 `resource`。以便覆盖更多场景。权限判断具体代码在 `@/utils/authentication.ts` 中，下面做简单讲解。
...
Ddmc Design Pro 封装了 `PermissionWrapper` 组件，可包裹固定元素进行权限管理
