...
After this upgrade, the component library name changed from ByteDesign to **DdmcDesign** and the package name changed to **@ddmc-design/web-react**.
...
```bash
npm remove @bytedesign/web-react && npm i @ddmc-design/web-react
```

2. Global replacement of all package names used in the project: @bytedesign/web-react -> @ddmc-design/web-react

> For this step, you can use the global replacement function of editor.

```diff
- import { Transfer } from '@bytedesign/web-react';
+ import { Transfer } from '@ddmc-design/web-react';

- import { IconRight, IconLeft } from '@bytedesign/web-react/icon';
+ import { IconRight, IconLeft } from '@ddmc-design/web-react/icon';

- @import '~@bytedesign/web-react/dist/css/byteui.css';
+ @import '~@ddmc-design/web-react/dist/css/byteui.css';

- import '@bytedesign/web-react/dist/css/byteui.css';
+ import '@ddmc-design/web-react/dist/css/byteui.css';
```

3. Replace CSS file name: byteui -> ddmc
...
CSS class name prefix of all components are changed from `byte-` to `ddmc-`. If there is style overlay of ByteDesign in your project, please follow the migration steps below.
...
After the above three steps, the component class name prefix and style prefix in DdmcDesign will be changed to `byte-`, **but we still reminds you that there are some changes of class name and Dom structure in DdmcDesign, please make sure whether there is style overridw on ByteDesign in the project, if there is, please check carefully whether the page performs normally in DdmcDesign.**

## Migration Tool

Since there are many **Breaking Change** in this large version updrade, in order to improve efficiency, we provide **codemod cli** tool `@ddmc-design/codemod` for you.
...
```bash
# 直接通过 npx 运行
npx -p @ddmc-design/codemod ddmc-codemod ./src

# 或者全局安装
npm i @ddmc-design/codemod -g
...
<img src="https://p1-ddmc.byteimg.com/tos-cn-i-uwbnlip3yd/de0edfe3ea17ee9be4c911fdb75355ef.gif~tplv-uwbnlip3yd-image.image" alt="codemod cli 运行过程" width="720"/>
...
- **Tag**
  -   Removed `fill`, DdmcDesign itself is graphic design
...
-   **Menu**
    -   Removed `activeItemBorderDirection` of `Menu`, DdmcDesign doesn't contain highlighted indication of the selected state
