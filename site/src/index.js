...
import { ConfigProvider } from '@ddmc-design/web-react';
...
export function Index() {
  const ddmcDirection = localStorage.getItem('ddmc-direction');
  const [user, setUser] = useState();
  const [noticeHeight, setNoticeHeight] = useState(0);
  const [rtl, setRtl] = useState(ddmcDirection === 'rtl');
  ...
  useEffect(() => {
    localStorage.setItem('ddmc-direction', rtl ? 'rtl' : 'ltr');
