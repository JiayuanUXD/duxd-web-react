export function goPath(language, setLocal) {
  ...
  if (language === 'en-US') {
    if (setLocal) {
      localStorage.setItem('ddmc-lang', 'en-US');
    }
    ...
    if (setLocal) {
      localStorage.setItem('ddmc-lang', 'zh-CN');
    }
...
export function resetI18nLocalStorage(lang) {
  const localLang = localStorage.getItem('ddmc-lang');
  if (localLang && localLang !== lang) {
    localStorage.setItem('ddmc-lang', lang);
  }
}
