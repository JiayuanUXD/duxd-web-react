...
import { IconSkin, IconLink, IconClose } from '@ddmc-design/web-react/icon';
...
function ThemeBox({ lang = 'zh-CN' }) {
  ...
  const themeObj = JSON.parse(localStorage.getItem('ddmc-components-theme'));
  ...
  async function fetchThemeList(current = currentPage, searchValue = keyword) {
    ...
    const data = await axios.get(
      `${apiBasename}/themes/api/open/themes/list?pageSize=6&currentPage=${current}&depLibrary=@ddmc-design/web-react&keyword=${searchValue}`
    );
    setLoading(false);
  ...
  function getCssLink(name: string, prefix: string) {
    return `${prefix}${name}/css/ddmc.css`;
  }

  function createCss(url: string) {
    const link = document.createElement('link');
    link.id = 'ddmc-custom-theme';
    link.href = url;
    link.type = 'text/css';
    link.rel = 'stylesheet';
  ...
  function resetTheme() {
    const customLink = document.getElementById('ddmc-custom-theme');
    if (customLink) {
      document.body.removeChild(customLink);
    }
    localStorage.setItem('ddmc-components-theme', '{}');
    setVisible(false);
  }
