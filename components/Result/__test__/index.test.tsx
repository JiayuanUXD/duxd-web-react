...
describe('Result', () => {
  it('render status correctly', () => {
    ...
    status.forEach((x) => {
      ...
      expect(wrapper.find('.ddmc-result')[0].classList.contains(`ddmc-result-is-${x}`)).toBe(true);
      expect(wrapper.find(`.ddmc-result-icon-${x}`)).toHaveLength(1);
      cleanup();
    });
  });
  ...
  it('render custom icon correctly', () => {
    ...
    expect(wrapper.find(`.ddmc-result-icon-info`)[0].innerHTML).toEqual('<span>custom icon</span>');
  });
});
