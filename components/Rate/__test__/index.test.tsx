...
function checkRateValue(component, value: number): boolean {
  if (!value) {
    return !component
      .find('.ddmc-rate-character')
      .item(0)
      .classList.contains(/ddmc-rate-character-[half|full]/);
  }
  ...
  return component
    .find('.ddmc-rate-character')
    .item(index)
    .classList.contains(`ddmc-rate-character-${isHalf ? 'half' : 'full'}`);
}
...
describe('Rate', () => {
  it('Click Rate not allow half', () => {
    ...
    fireEvent.click(component.find('.ddmc-rate-character-left').item(3));
  ...
  it('Click Rate allow half', () => {
    ...
    fireEvent.click(
      component.find('.ddmc-rate-character').item(3).querySelector('.ddmc-rate-character-left')!
    );
    expect(checkRateValue(component, 3.5)).toBe(true);

    fireEvent.click(
      component.find('.ddmc-rate-character').item(3).querySelector('.ddmc-rate-character-right')!
    );
    expect(checkRateValue(component, 4)).toBe(true);
  });
  ...
  it('onChange & onHoverChange is called', () => {
    ...
    const character = component
      .find('.ddmc-rate-character')
      .item(3)
      .querySelector('.ddmc-rate-character-left');
