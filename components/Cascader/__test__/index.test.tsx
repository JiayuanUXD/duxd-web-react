...
describe('Cascader basic test', () => {
  ...
  it('select correctly', async () => {
    ...
    expect(wrapper.querySelector('.ddmc-cascader-view-value')).toHaveTextContent('上海 / 上海市');
    // 消失有动画的延时
    jest.runAllTimers();
    expect(mockFn).toHaveBeenCalledWith(false); // 传入参数false
  });
  ...
  it('changeOnselect correctly', () => {
    ...
    expect(wrapper.querySelector('.ddmc-cascader-view-value')).toHaveTextContent('上海');
  });
});
...
describe('support clear correctly', () => {
  ...
  it('set defaultValue correctly', () => {
    ...
    expect(wrapper.find('.ddmc-cascader-list-column')).toHaveLength(2);
  });

  // 清除值正确
  it('clear value correctly', () => {
    fireEvent.click(wrapper.find('.ddmc-icon-close')[0]);
...
describe('support multiple correctly', () => {
  ...
  it('multiply select correctly', () => {
    ...
    act(() => {
      fireEvent.click(list.querySelector(`${prefixCls}-list-item .ddmc-checkbox `), {});
    });
    jest.runAllTimers();
    expect(value).toEqual([]);
    act(() => {
      fireEvent.click(
        list.querySelectorAll(`${prefixCls}-list-item`).item(1).querySelector('.ddmc-checkbox')
      );
      fireEvent.click(
        list.querySelectorAll(`${prefixCls}-list-item`).item(2).querySelector('.ddmc-checkbox')
      );
    });
  ...
  it('multiply select checkedStrategy correctly controlled', () => {
    ...
    expect(wrapper.find('.ddmc-tag > span')[0].textContent).toBe(`北京`);
    ...
    expect(wrapper.find('.ddmc-checkbox').length).toEqual(
      wrapper.find('.ddmc-checkbox-checked').length
    );
  ...
  it('multiply select checkedStrategy correctly', () => {
    ...
    expect(wrapper.find('.ddmc-tag > span').item(0).textContent).toBe(`北京-北京市-朝阳区`);

    fireEvent.click(wrapper.container.querySelector(prefixCls) as Element);
    jest.runAllTimers();
    fireEvent.click(document.querySelector('.ddmc-checkbox') as Element);
...
describe('fieldNames Cascader', () => {
  ...
  it('multiply select correctly', () => {
    ...
    expect(wrapper.find('.ddmc-tag').length).toBe(1);
    act(() => {
      fireEvent.click(wrapper.find(prefixCls)[0]);
    });

    expect(wrapper.find(`.ddmc-cascader-list-column`)).toHaveLength(4);
    const list = wrapper.find(`${prefixCls}-list-column`).item(2);

    act(() => {
      fireEvent.click(
        list.querySelectorAll(`${prefixCls}-list-item`).item(0).querySelector('.ddmc-checkbox')
      );
    });
