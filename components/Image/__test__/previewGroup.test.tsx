...
const srcList = [
  '//p1-ddmc.byteimg.com/tos-cn-i-uwbnlip3yd/dae45672685c4be6a297acc7848eab56~tplv-uwbnlip3yd-image.image',
  '//p1-ddmc.byteimg.com/tos-cn-i-uwbnlip3yd/a5eb077f42834139ad7ac17563056664~tplv-uwbnlip3yd-image.image',
  '//p1-ddmc.byteimg.com/tos-cn-i-uwbnlip3yd/5c030fe27c0e467a9a7d62c36ae4805b~tplv-uwbnlip3yd-image.image',
  '//p1-ddmc.byteimg.com/tos-cn-i-uwbnlip3yd/04b379e20b6c42f78d17cac99fcf4fd7~tplv-uwbnlip3yd-image.image',
];
...
describe('mount and unmount', () => {
  ...
  it('render basic group correctly', () => {
    ...
    expect(wrapper.find('.ddmc-image-preview')[0]).toBeTruthy();
  ...
  it('handle arrow click correctly', () => {
    ...
    const disabledArrowClass = 'ddmc-image-preview-arrow-disabled';

    expect(
      wrapper.find('.ddmc-image-preview-arrow-left')[0].classList.contains(disabledArrowClass)
    ).toBeTruthy();

    act(() => {
      fireEvent.click(wrapper.find('.ddmc-image-preview-arrow-right')[0]);
    });
    expect(
      wrapper.find('.ddmc-image-preview-arrow-left')[0].classList.contains(disabledArrowClass)
    ).toBeFalsy();
  ...
  it('should trigger change event correctly', () => {
    ...
    act(() => {
      fireEvent.click(wrapper.find('.ddmc-image-img')[1]);
      jest.runAllTimers();
    });
    ...
    act(() => {
      fireEvent.click(wrapper.find('.ddmc-image-preview-arrow-left')[0]);
      jest.runAllTimers();
    });
  ...
  it('handle group onVisibleChange correctly', () => {
    ...
    act(() => {
      fireEvent.click(wrapper.find('.ddmc-image-preview-close-btn')[0]);
    });
  ...
  it('transparent transmission of previewProps', () => {
    ...
    expect(wrapper.find('.ddmc-image-preview')[0].classList).toContain('preview-0');

    act(() => {
      fireEvent.load(wrapper.find('.ddmc-image-preview img')[0]);
      jest.runAllTimers();
    });
    ...
    expect(wrapper.find('.ddmc-image-preview')[0].classList).toContain('preview-1');
    act(() => {
      fireEvent.error(wrapper.find('.ddmc-image-preview img')[0]);
      jest.runAllTimers();
    });
