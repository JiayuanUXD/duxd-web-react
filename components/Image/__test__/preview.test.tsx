...
describe('Image', () => {
  ...
  const handleAction = (wrapper, index) => {
    const curElem = wrapper.find('.ddmc-image-preview-toolbar-action')[index];
    if (curElem) {
  ...
  it('handle scale event correctly', () => {
    ...
    jest.runAllTimers();
    expect(wrapper.find('.ddmc-image-preview-img-container')[0].style.transform).toEqual(
      beforeScale
    );
  ...
  it('handle rotate event correctly', async () => {
    const wrapper = render(<Image.Preview src={imgSrc} visible />);
    jest.runAllTimers();
    expect(wrapper.find('.ddmc-image-preview-img')[0].style.transform).toEqual(
  ...
  it('handle fullScreen event correctly', async () => {
    ...
    // 只能判断全屏按钮后scale与原值不同。
    expect(wrapper.find('.ddmc-image-preview-img-container')[0].style.transform).not.toEqual(
      `scale(1, 1)`
    );
  });
  ...
  it('handle maskClosable prop correctly', () => {
    ...
    act(() => {
      fireEvent.click(wrapper.find('.ddmc-image-preview-wrapper')[0]);
    });
  ...
  it('handle error src prop correctly', () => {
    ...
    jest.runAllTimers();
    expect(wrapper.find('.ddmc-icon-loading')).toHaveLength(1);
    act(() => {
      updateImg(wrapper, 'error');
    });
    expect(wrapper.find('.ddmc-image-preview-toolbar')).toHaveLength(0);
    ...
    expect(wrapper.find('.ddmc-image-preview-toolbar')).toHaveLength(1);
  });
  ...
  it('Mouse Event mouse start and mouse end', () => {
    ...
    expect(wrapper.find('.ddmc-image-preview-img-moving')[0]).toBeTruthy();
    ...
    expect(wrapper.find('.ddmc-image-preview-img-moving')[0]).toBeUndefined();
  });
