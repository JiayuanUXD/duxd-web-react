...
describe('Image', () => {
  ...
  it('render image with error src', () => {
    ...
    expect(wrapper.find('.ddmc-image-error')).toHaveLength(1);
    act(() => {
      wrapper.find('img')[0].setAttribute('src', imgSrc);
      updateImg(wrapper);
    });
    expect(wrapper.find('.ddmc-image-error')).toHaveLength(0);
  });
  ...
  it('render extra options correctly', () => {
    ...
    expect(wrapper.find('.ddmc-image-actions-list .ddmc-image-actions-item')).toHaveLength(1);
    act(() => {
