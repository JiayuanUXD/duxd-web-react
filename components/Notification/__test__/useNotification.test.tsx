...
describe('useNotification Test', () => {
  ...
  it('useNotification', async () => {
    ...
    expect($('.ddmc-notification')).toHaveLength(1);
    expect($('.ddmc-notification-content')[0].innerHTML).toBe('YYH');
  });
  ...
  it('notification update', async () => {
    ...
    expect(wrapper.find('.ddmc-notification-content')[0].innerHTML).toBe('PJY');
    notification.info?.({
      id: 'demo',
      content: 'YYH',
    });
    expect(wrapper.find('.ddmc-notification-content')[0].innerHTML).toBe('YYH');
  });
  ...
  it('useNotification getcontainer', async () => {
    ...
    expect(wrapper.find('#root .ddmc-notification')).toHaveLength(1);
  });
});
