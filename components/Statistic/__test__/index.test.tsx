...
describe('Statistic', () => {
  ...
  it('groupSeparator', () => {
    ...
    expect(
      component.find<HTMLSpanElement>('.ddmc-statistic-value > .ddmc-statistic-value-int')[0]
        .textContent
    ).toBe('100,000');
  ...
  it('prefix / suffix', () => {
    ...
    expect(
      component.find<HTMLSpanElement>('.ddmc-statistic-value > .ddmc-statistic-value-decimal')[0]
        .textContent
    ).toBe('%');
  });
  ...
  it('precision', () => {
    ...
    expect(
      component.find('.ddmc-statistic-value > .ddmc-statistic-value-int')[0].innerHTML
    ).toContain('1,000,000');
    expect(
      component.find<HTMLSpanElement>('.ddmc-statistic-value > .ddmc-statistic-value-decimal')[0]
        .textContent
  ...
  it('loading', async () => {
    ...
    expect(component.find('.ddmc-statistic-value > .ddmc-statistic-value-int ').length).toBe(0);
    expect(Number(component.find('.ddmc-skeleton').length)).toBe(1);
    ...
    expect(Number(component.find('.ddmc-skeleton').length)).toBe(0);
  });
  ...
  it('renderFormat support number value', () => {
    ...
    expect(component.find('.ddmc-statistic-value')[0].textContent).toEqual(
  ...
  it('renderFormat support number value', () => {
    ...
    expect(component.find('.ddmc-statistic-value')[0].textContent).toEqual(`${value}-${value}`);
  });
});
