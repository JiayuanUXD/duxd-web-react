...
describe('useMessage Test', () => {
  ...
  it('useMessage', async () => {
    ...
    expect($('.ddmc-message')).toHaveLength(1);
    expect($('.ddmc-message-content')[0].innerHTML).toBe('YYH');
    ...
    expect(wrapper.find('.ddmc-message').length).toBe(0);
  });
  ...
  it('message update', async () => {
    ...
    expect(wrapper.find('.ddmc-message-content')[0].innerHTML).toBe('PJY');

    const m = messageInstance.info?.({
      id: 'demo',
      content: 'YYH',
    });
    expect(wrapper.find('.ddmc-message-content')[0].innerHTML).toBe('YYH');
  ...
  it('should maintain instance stability during re-renders', async () => {
    ...
    const wrapper = render(<TestComponent />);
    expect($('.ddmc-message')).toHaveLength(0);
