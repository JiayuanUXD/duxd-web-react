...
describe('Breadcrumb', () => {
  it('breadcrumb separator correctly', () => {
    ...
    expect(wrapper.find('.ddmc-breadcrumb-item-separator').item(0).innerHTML).toBe('=&gt;');
  });

  it('breadcrumb render empty children correctly', () => {
    const wrapper = render(<Breadcrumb />);
    expect(wrapper.find('.ddmc-breadcrumb')).toHaveLength(1);
    expect(wrapper.find('.ddmc-breadcrumb-item')).toHaveLength(0);
  });
  ...
  it('supprt droplist correctly', () => {
    ...
    expect(wrapper.find('.ddmc-breadcrumb-item-with-dropdown')).toHaveLength(1);
    fireEvent.click(wrapper.getAllByRole('listitem')[1]);
    expect(wrapper.find('.ddmc-dropdown')).toHaveLength(1);
    expect(wrapper.find('.ddmc-dropdown')[0].className).toContain('ddmc-trigger-position-bl');
  });
  ...
  it('support routes correctly', () => {
    ...
    expect(wrapper.find('.ddmc-breadcrumb-item-with-dropdown')).toHaveLength(3);
    expect(wrapper.find('.ddmc-breadcrumb-item a').item(2).innerHTML).toEqual(`name-2`);

    expect(wrapper.find<HTMLAnchorElement>('.ddmc-breadcrumb-item a').item(2).hash).toEqual(
      '#/0/1/2'
    );
  });
  ...
  it('support maxCount correctly', () => {
    ...
    expect(wrapper.find('.ddmc-breadcrumb-item')).toHaveLength(3);
    expect(wrapper.find('.ddmc-breadcrumb-item-ellipses')).toHaveLength(1);

    maxCount = 5;
    const delta = routes.length - maxCount;
    wrapper.rerender(<Breadcrumb maxCount={maxCount} routes={routes} />);
    expect(wrapper.find('.ddmc-breadcrumb-item')).toHaveLength(5);
    expect(
      (wrapper.find('.ddmc-breadcrumb-item').item(3).firstChild! as HTMLAnchorElement).innerHTML
  ...
  it('support href correctly', () => {
    const wrapper = render(
      <Breadcrumb separator="=>">
        <BreadcrumbItem href="https://ddmc.design">Home</BreadcrumbItem>
        <BreadcrumbItem>Article</BreadcrumbItem>
        <BreadcrumbItem>Technology</BreadcrumbItem>
      </Breadcrumb>
    );
    expect(wrapper.find("a[href='https://ddmc.design']")).toHaveLength(1);
  });
  ...
  it('support custom Tag correctly', () => {
    ...
    expect(wrapper.find('.ddmc-btn')).toHaveLength(1);
    act(() => {
      fireEvent.click(wrapper.find('.ddmc-btn')[0]);
    });
