...
function getCells(component, listIndex) {
  return component
    ...
    .item(listIndex)
    .querySelectorAll('li.ddmc-timepicker-cell');
}

function getInputValue(component, index) {
  return component.find('.ddmc-picker-input input').item(index).getAttribute('value');
}
...
describe('TimePicker', () => {
  it('basic render', () => {
    ...
    expect(component.find('svg.ddmc-icon-clock-circle')).toHaveLength(1);

    fireEvent.click(component.find('.ddmc-picker')[0]);

    expect(component.find('.ddmc-timepicker-list')).toHaveLength(3);
    expect(component.find('.ddmc-timepicker-footer-btn-wrapper button')).toHaveLength(2);
  ...
  it('click TimePicker items should change active and call the right callback', () => {
    ...
    fireEvent.click(component.find('.ddmc-timepicker-footer-btn-wrapper button').item(1));
  ...
  //   component.find('.ddmc-picker-range').simulate('click');

  //   component
  //     .find('.ddmc-timepicker-list')
  //     .at(0)
  //     .find('li.ddmc-timepicker-cell')
  //     .at(1)
  //     .simulate('click');
  ...
  describe('use12hours', () => {
    it('use12Hours === true', () => {
      ...
      const time = document.querySelectorAll(
        '.ddmc-timepicker .ddmc-timepicker-list:first-child .ddmc-timepicker-cell:last-child .ddmc-timepicker-cell-inner'
      )[0].textContent;
  ...
  it('disabledHours / disabledMinutes / disabledSeconds', () => {
    ...
    expect(
      component
        .find('.ddmc-timepicker-list')
        .item(0)
        .querySelectorAll('.ddmc-timepicker-cell-disabled')
    ).toHaveLength(4);
  ...
  it('allowClear', () => {
    ...
    fireEvent.click(component.find('.ddmc-picker-clear-icon').item(0));

    expect(onChange.mock.calls).toHaveLength(1);
    expect(component.find('.ddmc-timepicker-container').length).toBe(0);
  });
  ...
  // https://github.com/ddmc-design/ddmc-design/issues/358
  ...
  // https://github.com/ddmc-design/ddmc-design/issues/494
