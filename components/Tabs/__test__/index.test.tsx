...
describe('Tabs', () => {
  it('onClickTab listener correctly', () => {
    ...
    const tabBars = wrapper.find('.ddmc-tabs-header-title');
    expect(tabBars[0].classList.contains('ddmc-tabs-header-title-active')).toBe(true);
    act(() => {
      fireEvent.click(tabBars[1]);
    });
    expect(
      wrapper.find('.ddmc-tabs-header-title')[1].classList.contains('ddmc-tabs-header-title-active')
    ).toBe(true);
  ...
  it('should show panel in right direction', () => {
    ...
    const tabBars = wrapper.find('.ddmc-tabs-content-inner');
  ...
  it('onEditTab ', () => {
    ...
    act(() => {
      fireEvent.click(wrapper.find('.ddmc-tabs-add-icon')[0]);
    });

    expect(tabs.length).toBe(6);
    wrapper.rerender(updateComp({}, getChildren()));

    expect(wrapper.find('.ddmc-tabs-header-title').length).toBe(6);
    expect(wrapper.find('.ddmc-tabs-content-item').length).toBe(6);

    fireEvent.click(wrapper.find('.ddmc-tabs-close-icon')[0]);
    ...
    expect(wrapper.find('.ddmc-tabs-header-title').length).toBe(5);
    expect(wrapper.find('.ddmc-tabs-content-item').length).toBe(5);
    ...
    expect(
      wrapper.find('.ddmc-tabs-header-title')[2].classList.contains('ddmc-tabs-header-title-active')
    ).toBe(false);
    ...
    expect(wrapper.find('.ddmc-tabs-header-title-active')[0]).toHaveTextContent('标签2');
    act(() => {
      fireEvent.click(wrapper.find('.ddmc-tabs-close-icon')[0]);
      wrapper.rerender(updateComp({}, getChildren()));
    });

    expect(wrapper.find('.ddmc-tabs-content-item').length).toBe(getChildren().length);

    expect(
      wrapper.find('.ddmc-tabs-header-title')[0].classList.contains('ddmc-tabs-header-title-active')
    ).toBe(false);
  });
});
