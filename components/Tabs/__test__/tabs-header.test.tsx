...
describe('Tabs Header Scroll', () => {
    ...
    Object.defineProperty(HTMLElement.prototype, 'offsetWidth', {
      get() {
        ...
        if (typeof className === 'string') {
          if (className.includes('ddmc-tabs-header-wrapper')) {
            return 100;
          }
          if (
            className.includes('ddmc-tabs-header') &&
            !className.includes('ddmc-tabs-header-wrapper')
          ) {
            return 300;
          }
        }
  ...
  it('Tabs Header replace dropdown icon', () => {
    ...
    expect(
      wrapper
        .find('.ddmc-tabs-header-scroll')[0]
        .className.includes('ddmc-tabs-header-overflow-dropdown')
    ).toBe(true);

    expect(wrapper.find('.ddmc-tabs-dropdown-icon')[0].innerHTML).toEqual(dropdownStr);
  });
  ...
  it('Tabs Header replace scroll icon', () => {
    ...
    expect(
      wrapper
        .find('.ddmc-tabs-header-scroll')[0]
        .className.includes('ddmc-tabs-header-overflow-scroll')
    ).toBe(true);

    expect(wrapper.find('.ddmc-tabs-left-icon>.ddmc-icon-edit')).toHaveLength(1);
    expect(wrapper.find('.ddmc-tabs-right-icon>.ddmc-icon-right')).toHaveLength(1);
  });
  ...
  it('Tabs Header without scrollIcon', () => {
    ...
    expect(wrapper.find('.ddmc-tabs-left-icon')).toHaveLength(0);
    expect(wrapper.find('.ddmc-tabs-right-icon')).toHaveLength(0);
  });
