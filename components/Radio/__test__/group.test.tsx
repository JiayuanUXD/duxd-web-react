...
describe('Radio.Group', () => {
  function test(options, isChildren) {
    ...
    it('expect render correctly', () => {
      ...
      expect(radioGroup.find('.ddmc-radio').length).toBe(3);

      expect(radioGroup.find('.ddmc-radio')[0]).toHaveClass('ddmc-radio-checked');
    });
    it('expect onChange correctly', () => {
      ...
      fireEvent.click(radioGroup.find('.ddmc-radio')[2]);
    ...
    it('value control', () => {
      ...
      fireEvent.click(radioGroup.find('.ddmc-radio')[1]);
      ...
      expect(radioGroup.find('.ddmc-radio')[1]).toHaveClass('ddmc-radio-checked');
      ...
      expect(radioGroup.find('.ddmc-radio')[2]).toHaveClass('ddmc-radio-checked');
    });
  }
  ...
  it('test disabled props', () => {
    ...
    expect(wrapper.find('.ddmc-radio')[2]).toHaveClass('ddmc-radio-disabled');

    expect(wrapper.find('.ddmc-radio')[1]).toHaveClass('ddmc-radio-checked');

    fireEvent.click(wrapper.find('.ddmc-radio')[0]);
    expect(wrapper.find('.ddmc-radio')[0]).toHaveClass('ddmc-radio-checked');
    expect(mockFn).toBeCalledTimes(1);

    fireEvent.click(wrapper.find('.ddmc-radio')[2]);
    expect(wrapper.find('.ddmc-radio')[0]).toHaveClass('ddmc-radio-checked');
