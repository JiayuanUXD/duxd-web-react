...
describe('Tag', () => {
  ...
  it('close tag', () => {
    ...
    fireEvent.click(tag.find('.ddmc-icon-close')[0]);
  ...
  it('async onClose is called while resolve', () => {
    ...
    fireEvent.click(tag.find('.ddmc-icon-close')[0]);
    expect(tag.find('.ddmc-tag')[0].className).toContain('ddmc-tag-loading');
    ...
    return onClose.mock.results[0].value
      ...
      .finally(() => {
        expect(tag.find('.ddmc-tag')[0].className).not.toContain('ddmc-tag-loading');
        expect(tag.find('.ddmc-tag')[0].className).toContain('ddmc-tag-hide');
      });
  });
  ...
  it('async onClose is called while reject', () => {
    ...
    return onClose.mock.results[0].value
      ...
      .finally(() => {
        expect(tag.find('.ddmc-tag')[0].className).not.toContain('ddmc-tag-loading');
        expect(tag.find('.ddmc-tag')[0].className).not.toContain('ddmc-tag-hide');
      });
  });
});
