...
describe('Upload api callbacks', function () {
  ...
  it('onPreview should be fired', async function () {
    ...
    expect(wrapper.find('.ddmc-upload-list-preview-icon')).toHaveLength(1);
    await act(() => {
      fireEvent.click(wrapper.find('.ddmc-upload-list-preview-icon').item(0));
    });
  ...
  describe('onRemove upload', () => {
    ...
    it('should delete', async () => {
      ...
      // onRemove 不设置，直接删除
      const items = wrapper.find('.ddmc-upload-list-item');
      expect(items).toHaveLength(2);

      await act(() => {
        fireEvent.click(wrapper.find('.ddmc-upload-list-remove-icon').item(0));
      });
  ...
  it('limit hide ', async function () {
    ...
    expect(wrapper.find('.ddmc-upload-trigger')).toHaveLength(0);
  });
  ...
  it('limit disabled ', async function () {
    ...
    await sleep(200);
    expect(wrapper.find('.ddmc-upload-trigger')).toHaveLength(1);
    expect(wrapper.find('.ddmc-upload-trigger .ddmc-btn-disabled')).toHaveLength(1);
  });

  it('accept xxx/* strict=true', async function () {
    const mockFn = jest.fn();
    const wrapper = render(<Upload multiple accept=".txt" action="/sss" onChange={mockFn} />);
    const triggerNode = wrapper.find('.ddmc-btn');
    ...
    expect(wrapper.find('.ddmc-upload-list-item')).toHaveLength(1);
  });
  ...
  it('accept xxx/* strict=false', async function () {
    ...
    expect(wrapper.find('.ddmc-upload-list-item')).toHaveLength(2);
  });
});
