...
describe('Tooltip', () => {
  ...
  it('mouseenter and mouseleave', () => {
    ...
    jest.runAllTimers();
    expect(wrapper.find('.ddmc-trigger').length).toBe(1);
    fireEvent.mouseLeave(screen.getByRole('button'));
    jest.runAllTimers();
    expect(wrapper.find('.ddmc-trigger').length).toBe(0);
  });
  ...
  it('does not show tooltip when content is null or undefined or false or "  "', () => {
    [false, undefined, null, '  '].forEach((item) => {
      ...
      fireEvent.mouseEnter(screen.getByRole('button'));
      jest.runAllTimers();
      expect(wrapper.find('.ddmc-tooltip-content-inner').length).toBe(0);
      cleanup();
    });
  });
  it('should show tooltip when content is 0', () => {
    ...
    fireEvent.mouseEnter(screen.getByRole('button'));
    jest.runAllTimers();
    expect(wrapper.find('.ddmc-tooltip-content-inner').length).not.toBe(0);
  });
  ...
  it('showArrow property', () => {
    ...
    expect(wrapper.find('.ddmc-trigger-arrow')).toHaveLength(0);
  });
  it('arrowProps property', async () => {
    ...
    fireEvent.mouseEnter(screen.getByRole('button'));
    jest.runAllTimers();
    expect(container.querySelector('.ddmc-trigger-arrow')?.style.backgroundColor).toBe(
      'rgb(255, 255, 255)'
    );
  });
  ...
  it('position', () => {
    ...
    positions.forEach((position) => {
      ...
      fireEvent.mouseEnter(screen.getByRole('button'));
      jest.runAllTimers();
      expect(wrapper.find('.ddmc-trigger')[0].className).toContain(
        `ddmc-trigger-position-${position}`
      );
      cleanup();
    });
  });
