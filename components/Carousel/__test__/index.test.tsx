...
const checkActiveIndex = (wrapper, index) => {
  const indicatorClassName = 'ddmc-carousel-indicator-item';
...
describe('Carousel', () => {
  ...
  it('miniRender works correctly', () => {
    ...
    expect(wrapper.find('.ddmc-carousel-slide > div')).toHaveLength(3);
  });
  ...
  it('should move correctly', () => {
    ...
    fireEvent.click(wrapper.querySelector('.ddmc-carousel-arrow-right'));
    checkActiveIndex(wrapper, 1);

    // 等待向右的动画结束，否则下次点击会被忽略
    jest.advanceTimersByTime(1000);

    fireEvent.click(wrapper.querySelector('.ddmc-carousel-arrow-left'));
  ...
  it('should selectIndex correctly', () => {
    ...
    fireEvent.click(wrapper.find('.ddmc-carousel-indicator-item').item(2));
  ...
  it('should addInterval, clearInterval correctly', () => {
    ...
    fireEvent.mouseEnter(document.querySelector('.ddmc-carousel'));
    jest.advanceTimersByTime(4000);
    checkActiveIndex(wrapper, 0);

    fireEvent.mouseLeave(document.querySelector('.ddmc-carousel'));
    jest.advanceTimersByTime(4000);
    checkActiveIndex(wrapper, 1);
  });
  ...
  it('should be dots', () => {
    ...
    expect(wrapper.find('.ddmc-carousel-indicator-dot')).toHaveLength(1);
    expect(wrapper.find('.ddmc-carousel-indicator-right')).toHaveLength(1);
    expect(wrapper.find('.ddmc-carousel-arrow')[0]).toHaveClass('ddmc-carousel-arrow-hover');
  });
  ...
  it('support the ref trigger change scene', async () => {
    ...
    expect(dom.className.includes('ddmc-carousel ')).toBeTruthy();
  ...
  it('support slider indicatorType correctly', () => {
    ...
    expect(wrapper.find('.ddmc-carousel-indicator-slider')).toHaveLength(1);
    jest.advanceTimersByTime(3000);

    const activeIndicator = document.querySelector(
      '.ddmc-carousel-indicator-slider .ddmc-carousel-indicator-item'
    ) as HTMLElement;
