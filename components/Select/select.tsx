...
function Select(baseProps: SelectProps, ref) {
  ...
  if (mode === 'tags') {
    ...
    warning(
      true,
      `[Ddmc Select] The 'tags' mode will be removed in the next major version, please use {mode: "multiple", allowCreate: true} instead.`
    );
  }
  ...
  const getValueAndOptionForCallback = (
    ...
    if (labelInValue && !isEmpty) {
      const getOptionLabel = (optionValue: OptionProps['value'], optionInfo: OptionInfo) => {
        ...
        // https://github.com/ddmc-design/ddmc-design/issues/442
