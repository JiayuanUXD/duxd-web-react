...
function Row(props: SummaryRowProps) {
  ...
  const element = React.Children.map(children, (child, index) => {
    ...
    // childElement?.props?.$$DdmcTableSummaryCell: Compatible Cell.defaultProps.$$DdmcTableSummaryCell
    const isSummaryCell =
      get(childElement, 'type.__DDMC_TABLE_SUMMARY_CELL__') ||
      get(childElement, 'props.$$DdmcTableSummaryCell');
