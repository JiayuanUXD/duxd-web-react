...
describe('Virtualized Table', () => {
  it('render virtualized table correctly', () => {
    ...
    expect(component.find('.ddmc-table-body div.ddmc-table-tr')).toHaveLength(5);

    component.rerender(<Table columns={columns} data={[]} virtualized />);

    expect(
      component.find('.ddmc-table-body table .ddmc-table-empty-row td')[0].getAttribute('colSpan')
    ).toBe('5');
  });
