...
function simulateFilterCheckbox(filterItem, index: number) {
  // 点击过滤项
  fireEvent.click(filterItem.item(index).querySelector('.ddmc-checkbox > input'));
}
...
describe('Table Filter', () => {
  it('filter', async () => {
    function checkFilter(columns: ColumnProps<TestData>[]) {
      ...
      const filter = component.find('div.ddmc-table-filters')[0];

      expect(filter.className).toContain('ddmc-table-filters-active');
      expect(component.find('tbody tr')).toHaveLength(3);

      fireEvent.click(filter);
      const filterItem = component.find('.ddmc-table-filters-item');
      expect(filterItem).toHaveLength(2);

      simulateFilterCheckbox(filterItem, 1); // 选中Female

      const okButton = component.find('.ddmc-table-filters-btn .ddmc-btn');
  ...
  it('custom filter', async () => {
    ...
    const filter = component.find('.ddmc-table-filters')[0];

    expect(filter.querySelector('svg')?.classList.contains('ddmc-icon-search')).toBe(true);
    expect(component.find('tbody tr')).toHaveLength(5);

    fireEvent.click(filter);

    const customFilterPopup = component.find('.ddmc-table-custom-filter')[0];
    fireEvent.click(customFilterPopup.querySelectorAll('.ddmc-btn').item(0));
    fireEvent.click(customFilterPopup.querySelectorAll('.ddmc-btn').item(1));
  ...
  it('auto reset pagination current when filter', () => {
    ...
    expect(component.find('.ddmc-pagination-item')).toHaveLength(5);
    expect(component.find('.ddmc-pagination-item-active')[0].textContent).toBe('1');

    fireEvent.click(component.find('.ddmc-pagination-item').item(3));

    expect(component.find('.ddmc-pagination-item-active')[0].textContent).toBe('3');
    ...
    expect(component.find('.ddmc-pagination-item')).toHaveLength(4);
    expect(component.find('.ddmc-pagination-item-active')[0].textContent).toBe('1');
  });
