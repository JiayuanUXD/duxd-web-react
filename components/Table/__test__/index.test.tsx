...
describe('Table', () => {
  ...
  it('render table correctly', () => {
    ...
    expect(th.item(0).querySelectorAll('.ddmc-table-th-item-title')[0].textContent).toBe('Name');
    expect(td.item(0).querySelectorAll('.ddmc-table-cell')[0].textContent).toBe('Name1');
  });
  ...
  it('table no data', () => {
    ...
    expect(component.find('table tbody td .ddmc-empty')).toHaveLength(1);

    component.rerender(<Table columns={columns} data={[]} noDataElement="No Data" />);

    expect(component.find('table tbody td .ddmc-table-no-data')[0].textContent).toBe('No Data');
  });

  it('table loading', () => {
    const component = render(<Table columns={columns} data={[]} />);

    expect(component.find('.ddmc-spin .ddmc-spin-icon')).toHaveLength(0);

    component.rerender(<Table columns={columns} data={[]} loading />);

    expect(component.find('.ddmc-spin .ddmc-spin-icon')).toHaveLength(1);
  });

  it('table size', () => {
    const component = render(<Table size="small" columns={columns} data={[]} />);

    expect(component.find('.ddmc-table')[0].classList.contains('ddmc-table-size-small')).toBe(true);

    component.rerender(<Table size="middle" columns={columns} data={[]} />);

    expect(component.find('.ddmc-table')[0].classList.contains('ddmc-table-size-middle')).toBe(
      true
    );
  });
  ...
  it('table cell custom render', () => {
    ...
    expect(component.find('tbody td .ddmc-tag')).toHaveLength(5);
  });
