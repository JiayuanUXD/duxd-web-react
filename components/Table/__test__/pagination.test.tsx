...
describe('Table test', () => {
  ...
  it('table pagination', () => {
    ...
    const getFirstName = () => {
      return component.find('tbody td').item(0).querySelector('.ddmc-table-cell')?.textContent;
    };

    expect(component.find('.ddmc-pagination-list')[0].children).toHaveLength(5);
    expect(getFirstName()).toBe('Name1');
    fireEvent.click(component.find('.ddmc-pagination-list .ddmc-pagination-item').item(2));
    ...
    expect(component.find('.ddmc-pagination')).toHaveLength(0);
  });
  ...
  it('when no data, but pagination.total > 0, should show pagination list', () => {
    ...
    expect(component.find('.ddmc-table-no-data')).toHaveLength(1);
    expect(component.find('.ddmc-table-pagination')).toHaveLength(1);
  });
