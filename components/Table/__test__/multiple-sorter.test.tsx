  ...
  it('multiple sorter', () => {
    ...
    function getRowData(index: number) {
      ...
      for (let i = 0; i < length; i++) {
        const value =
          trs.item(i).querySelectorAll('td .ddmc-table-cell').item(index).textContent || '';
    ...
    expect(ths.item(2).classList).toContain('ddmc-table-col-sorted');
    expect(ths.item(3).classList).toContain('ddmc-table-col-sorted');
    expect(ths.item(4).classList).toContain('ddmc-table-col-sorted');

    const nameSorter = component.find('.ddmc-table-cell-with-sorter').item(0);
    const ageSorter = component.find('.ddmc-table-cell-with-sorter').item(1);
    const scoreASorter = component.find('.ddmc-table-cell-with-sorter').item(2);
    const scoreBSorter = component.find('.ddmc-table-cell-with-sorter').item(3);
    const scoreCSorter = component.find('.ddmc-table-cell-with-sorter').item(4);
    ...
    expect(component.querySelectorAll('thead .ddmc-table-col-sorted')).toHaveLength(1);
