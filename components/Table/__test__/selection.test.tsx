...
describe('Table selection', () => {
  it('checkbox', () => {
    ...
    expect(component.find('.ddmc-checkbox').length).toEqual(trLength);
    expect(component.find('.ddmc-checkbox-disabled')).toHaveLength(1);

    const checkboxBtns = component.find('.ddmc-checkbox > input');
    ...
    expect(component.find('.ddmc-checkbox-checked')).toHaveLength(5);

    fireEvent.click(checkAllBtn);

    expect(onChange.mock.calls.length).toBe(2);
    expect(component.find('.ddmc-checkbox-checked')).toHaveLength(0);

    fireEvent.click(checkboxBtns.item(1));

    expect(onChange.mock.calls.length).toBe(3);
    expect(component.find('.ddmc-checkbox-checked')).toHaveLength(1);
    expect(
      component.find('tbody tr').item(0).querySelectorAll('.ddmc-checkbox-checked')
    ).toHaveLength(1);
  });
  ...
  it('radio', () => {
    ...
    expect(component.find('colgroup col')).toHaveLength(6);
    expect(component.find('.ddmc-radio').length).toBe(trLength - 1);
    expect(component.find('.ddmc-radio-disabled')).toHaveLength(1);
    expect(component.find('table thead th.ddmc-table-radio')[0].textContent).toBe('columnTitle');

    fireEvent.click(component.find('.ddmc-radio input').item(0));

    expect(onChange.mock.calls.length).toBe(1);
    expect(component.find('.ddmc-radio.ddmc-radio-checked')).toHaveLength(1);
    expect(
      component.find('tbody tr').item(0).querySelectorAll('.ddmc-radio.ddmc-radio-checked')
    ).toHaveLength(1);

    fireEvent.click(component.find('.ddmc-radio input').item(2));

    expect(onChange.mock.calls.length).toBe(2);
    expect(component.find('.ddmc-radio.ddmc-radio-checked')).toHaveLength(1);
    expect(
      component.find('tbody tr').item(2).querySelectorAll('.ddmc-radio.ddmc-radio-checked')
    ).toHaveLength(1);
  });
  ...
  it('rowSelection.checkCrossPage', () => {
    ...
    fireEvent.click(component.find('.ddmc-checkbox > input').item(0));

    expect(onChange.mock.calls[0][0]).toEqual(['Name1', 'Name2']);

    fireEvent.click(component.find('.ddmc-pagination-item').item(2));
    ...
    fireEvent.click(component.find('.ddmc-pagination-item').item(1));
  ...
  it('rowSelection.renderCell type = checkbox', () => {
    ...
    expect(component.find('tbody .ddmc-table-checkbox').item(0).textContent).toBe('Name1');
  });
  ...
  it('rowSelection.renderCell type = radio', () => {
    ...
    expect(component.find('tbody .ddmc-table-radio').item(0).textContent).toBe('Name1');
  });
  ...
  it('rowSelection.checkStrictly', () => {
    ...
    // 1-1
    fireEvent.click(component.find('.ddmc-checkbox > input').item(2));

    function checkChecked(statusList) {
      statusList.forEach((status, i) => {
        expect(
          component.find('.ddmc-checkbox').item(i).classList.contains('ddmc-checkbox-checked')
        ).toBe(status);
      });
    }
    ...
    function checkIndeterminate(statusList) {
      statusList.forEach((status, i) => {
        expect(
          component.find('.ddmc-checkbox').item(i).classList.contains('ddmc-checkbox-indeterminate')
        ).toBe(status);
      });
    }
    ...
    // 1-2
    fireEvent.click(component.find('.ddmc-checkbox > input').item(5));
    ...
    // 1-1-1
    fireEvent.click(component.find('.ddmc-checkbox > input').item(3));
  ...
  it('rowSelection.checkConnected set selectedRowKeys', () => {
    ...
    // 1
    fireEvent.click(component.find('.ddmc-checkbox > input').item(1));
