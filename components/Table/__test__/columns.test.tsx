...
describe('Table columns usage test', () => {
  ...
  it('rowSelection change to undefined', () => {
    ...
    expect(component.find('.ddmc-checkbox')).toHaveLength(0);

    component.rerender(<Table columns={columns} data={data} rowSelection={{}} />);

    expect(component.find('.ddmc-checkbox')).toHaveLength(6);
  });
  ...
  it('set columns async, fixed columns correctly', () => {
    ...
    expect(
      component.find('.ddmc-table').item(0).className.indexOf('ddmc-table-has-fixed-col-left') > -1
    ).toBeTruthy();
    expect(
      component.find('.ddmc-table').item(0).className.indexOf('ddmc-table-has-fixed-col-right') > -1
    ).toBeTruthy();
  });
  ...
  it('delete column, tree data first column correctly', () => {
    ...
    function checkExpandIcon() {
      ...
      expect(component.find('td').item(0).querySelectorAll('.ddmc-icon-plus')).toHaveLength(1);
    }
