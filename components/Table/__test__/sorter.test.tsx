...
describe('Table sorter', () => {
  it('sorter', () => {
    function checkSorter(columns: ColumnProps<TestData>[]) {
      ...
      const sorter = component.find('.ddmc-table-cell-with-sorter')[0];
      const getFirstTdText = () => {
        return component.find('tbody tr').item(0).querySelectorAll('td .ddmc-table-cell').item(3)
          .textContent;
      };
  ...
  it('showSorterTooltip', async (done) => {
    ...
    fireEvent.mouseEnter(component.find('.ddmc-table-cell-with-sorter')[0]);

    function checkTooltipText(text: string) {
      setTimeout(() => {
        expect(document.querySelector('.ddmc-tooltip-content')!.innerHTML).toEqual(
          `<div class="ddmc-tooltip-content-inner">${text}</div>`
        );
        done();
      }, 200);
    }
    ...
    await sleep(200);
    fireEvent.click(component.find('.ddmc-table-cell-with-sorter')[0]);
    ...
    setTimeout(() => {
      expect(document.querySelector('.ddmc-tooltip-content')).toHaveLength(0);
      done();
    }, 200);
  });
  ...
  it('sort tree data', () => {
    ...
    const getAge = (index) => {
      return component.find('tbody tr').item(index).querySelectorAll('td .ddmc-table-cell').item(3)
        .textContent;
    };
