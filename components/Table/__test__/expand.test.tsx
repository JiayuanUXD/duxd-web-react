...
describe('Table expand', () => {
  it('basic expand', () => {
    ...
    expect(component.find('table td.ddmc-table-expand-icon-cell > button')).toHaveLength(4);

    fireEvent.click(component.find('table td.ddmc-table-expand-icon-cell > button').item(1));
    ...
    expect(component.find('tr.ddmc-table-expand-content')).toHaveLength(1);
  ...
  it('expandProps', () => {
    ...
    expect(firstCol.className).toContain('ddmc-table-expand-icon-col');
    expect(getComputedStyle(firstCol).getPropertyValue('width')).toBe('120px');

    expect(
      component.find('thead th').item(0).querySelector('.ddmc-table-th-item')?.textContent
    ).toBe('Expand Col');

    const getFirstExpandBtn = () =>
      component.find('table td.ddmc-table-expand-icon-cell > button').item(0);
  ...
  it('tree data', () => {
    ...
    expect(component.find('.ddmc-icon-plus')).toHaveLength(1);

    expect(component.find('.ddmc-checkbox')).toHaveLength(3);
  });
  ...
  it('expandProps.rowExpandable', () => {
    ...
    expect(component.find('.ddmc-icon-plus')).toHaveLength(4);
    expect(component.find('tr').item(2).querySelectorAll('.ddmc-icon-right')).toHaveLength(0);
  ...
  it('expandProps.strictTreeData', () => {
    ...
    expect(component.find('.ddmc-icon-plus')).toHaveLength(2);
  });
});
