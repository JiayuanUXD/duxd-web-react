...
describe('Progress', () => {
  it('render correctly ', () => {
    ...
    expect(wrapper.querySelector('.ddmc-progress-line-text')!.innerHTML).toBe('10%');
    wrapper.rerender(<Progress percent={20} />);
    expect(wrapper.querySelector('.ddmc-progress-line-text')!.innerHTML).toBe('20%');

    expect(wrapper.querySelector('.ddmc-progress-line-inner')!.style.width).toBe('20%');
  });

  it('render correctly when width props is string', () => {
    const wrapper = render(<Progress percent={10} width="100%" />);
    expect(wrapper.find('.ddmc-progress').item(0).style.width).toBe('100%');
  });

  it('render correctly when width props is number', () => {
    const wrapper = render(<Progress percent={10} width={200} />);
    expect(wrapper.find('.ddmc-progress').item(0).style.width).toBe('200px');
  });
  ...
  it('render correctly when trailColor props is exist & type is circle', () => {
    ...
    expect(wrapper.find('.ddmc-progress-circle-mask')[0].style.stroke).toBe('rgb(0,0,0)');
  });
});
