...
describe('Timeline', () => {
  it('Prop horizontal & reverse works', () => {
    ...
    expect(wrapper.find('.ddmc-timeline').item(0).className).toContain(
      'ddmc-timeline-direction-horizontal'
    );

    expect(wrapper.find('.ddmc-timeline-item-content').item(0).innerHTML).toBe(
      'The third milestone'
    );
  });
  ...
  it('Ignore invalid child', () => {
    ...
    expect(wrapper.find('.ddmc-timeline-item')).toHaveLength(3);
  });
  ...
  it('Pending status', () => {
    ...
    expect(
      wrapper.find('.ddmc-timeline-item .ddmc-timeline-item-dot-line').item(2).style.borderLeftStyle
    ).toBe('dashed');
  });
});
