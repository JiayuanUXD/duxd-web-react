...
describe('Input', () => {
  ...
  it('prefix, suffix, addBefore, addAfter correctly', () => {
    ...
    expect(component.find('.ddmc-input-group-addafter')).toHaveLength(1);
    expect(component.find('.ddmc-input-group-addbefore')).toHaveLength(1);
    expect(component.find('.ddmc-input-group-prefix')).toHaveLength(1);
    expect(component.find('.ddmc-input-group-suffix')).toHaveLength(1);
    expect(component.find('.ddmc-input-group-addbefore > span')[0].textContent).toBe('addBefore');
    expect(component.find('.ddmc-input-group-addafter > span')[0].textContent).toBe('addAfter');
    expect(component.find('.ddmc-input-group-prefix > span')[0].textContent).toBe('prefix');
    expect(component.find('.ddmc-input-group-suffix > span')[0].textContent).toBe('suffix');
  });
});
...
describe('Test Search', () => {
  it('input search (searchButton)', () => {
    ...
    expect(component.querySelector('button svg')).toHaveClass('ddmc-icon-search');
  ...
  it('input search (icon)', () => {
    ...
    const iconButton = component.find('.ddmc-icon-search');
...
describe('Test Textarea', () => {
  ...
  it('test onClear', () => {
    ...
    fireEvent.click(component.find('.ddmc-input-clear-icon svg')[0]);
  ...
  it('test contains together allowClear and readOnly and do not show the clear button', () => {
    ...
    expect(component.find('.ddmc-input-clear-icon svg')).toHaveLength(0);
  });

  it('test password', () => {
    const component = mountInputPassword(<Input.Password visibilityToggle />);

    expect(component.find('svg')[0]).toHaveClass('ddmc-icon-eye-invisible');
    expect(component.find('input')[0].getAttribute('type')).toBe('password');
    fireEvent.click(component.find('svg')[0]);
    expect(component.find('svg')[0]).toHaveClass('ddmc-icon-eye');
  ...
  it('test maxLength and boolean showWordLimit', () => {
    ...
    const inputLimitElement = () => input.container.querySelectorAll('.ddmc-input-word-limit');
    ...
    const textareaLimitElement = () =>
      textarea.container.querySelectorAll('.ddmc-textarea-word-limit');
  ...
  it('test TextArea wordLimitPosition', () => {
    ...
    const textareaWrapperElement = () => textarea.container.querySelector('.ddmc-textarea-wrapper');
    const textareaLimitElement = () =>
      textarea.container.querySelector('.ddmc-textarea-word-limit');
    expect(textareaWrapperElement()).toHaveClass('ddmc-textarea-wrapper-word-limit-outside');
    expect(textareaLimitElement()).toHaveClass('ddmc-textarea-word-limit-outside');
  ...
  it('test maxLength.errorOnly', () => {
    ...
    const inputLimitElement = () => input.find('.ddmc-input-word-limit');
    expect(inputLimitElement()[0].textContent).toEqual('20/10');
    expect(inputLimitElement()[0]).toHaveClass('ddmc-input-word-limit-error');
    ...
    expect(textareaLimitElement()).toHaveClass('ddmc-textarea-word-limit-error');
  });

  it('test Input clearIcon=xxx', () => {
    const component = mountInput(<Input allowClear clearIcon="xxx" defaultValue="123" />);
    expect(component.find('.ddmc-input-clear-icon').item(0).textContent).toBe('xxx');
  });

  it('test TextArea clearIcon=xxx', () => {
    const component = mountInput(<Input.TextArea allowClear clearIcon="xxx" defaultValue="123" />);
    expect(component.find('.ddmc-textarea-clear-icon').item(0).textContent).toBe('xxx');
  });
