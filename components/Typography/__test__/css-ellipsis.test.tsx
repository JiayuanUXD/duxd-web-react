...
describe('Css Ellipsis Scene', () => {
  ...
  it('support basic ellipsis', async () => {
    ...
    expect(wrapper.find('.ddmc-typography-simple-ellipsis')).toHaveLength(1);
    const ellipsisStyle1 = wrapper.find('.ddmc-typography-simple-ellipsis > span')[0].style;
    ...
    const ellipsisStyle2 = wrapper.find('.ddmc-typography-simple-ellipsis > span')[0].style;
    // @ts-ignore
  ...
  it('support expand when ellipsis', async () => {
    ...
    expect(wrapper.find('.ddmc-typography-operation-expand')[0]).toHaveTextContent('More');
    act(() => {
      fireEvent.click(wrapper.find('.ddmc-typography-operation-expand')[0]);
    });

    expect(wrapper.find('.ddmc-typography')[0]).toHaveTextContent(`${mockText}Less`);
    ...
    expect(wrapper.find('.ddmc-typography-expand')).toHaveLength(0);
  });
  ...
  it('support tooltip when ellipsis', async () => {
    ...
    expect(wrapper.find('.ddmc-typography')[0]).toHaveTextContent(mockText);
    expect(wrapper.find('.ddmc-typography-simple-ellipsis')).toHaveLength(1);
    act(() => {
      // @ts-ignore
      fireEvent.mouseEnter(wrapper.find('.ddmc-typography')[0].closest('span'));
    });
    jest.runAllTimers();
    await sleep(200);
    expect(wrapper.find('.ddmc-tooltip-open')).toHaveLength(1);
  });
  ...
  it('ellipsis correctly when children is controlled', async () => {
    ...
    expect(wrapper.find('.ddmc-typography')[0]).toHaveTextContent(resetText + mockText);
  });
  ...
  it('ellipsis controlled scene', async () => {
    ...
    await sleep(200);
    expect(wrapper.find('.ddmc-typography')[0]).toHaveTextContent(
    ...
    expect(
      // @ts-ignore
      wrapper.find('.ddmc-typography-simple-ellipsis > span')[0].style?.WebkitLineClamp
    ).toEqual('2');
  });
  ...
  it('support defaultEllipsis scene', async () => {
    ...
    expect(wrapper.find('.ddmc-typography-simple-ellipsis')).toHaveLength(0);
  });
  ...
  it('render expanding operation correctly', async () => {
    ...
    await sleep(200);
    expect(wrapper.find('.ddmc-typography-operation-expand')[0]).toHaveTextContent('展开');
    ...
    expect(wrapper.find('.ddmc-typography-operation-expand')).toHaveLength(0);
  });
  ...
  it('support ellipsis in code mode', () => {
    ...
    expect(wrapper.find('.ddmc-typography')[0]).toHaveTextContent(`${mockText}展开`);
    expect(wrapper.find('code')[0].classList.contains('ddmc-typography-simple-ellipsis')).toEqual(
      true
    );
  });
});
