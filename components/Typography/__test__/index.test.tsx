...
describe('Typography', () => {
  it('basic bold', () => {
    ...
    expect(wrapper.find('.ddmc-typography')).toHaveLength(1);
  ...
  it('specal Paragraph render', () => {
    ...
    expect(wrapper.find('.ddmc-typography')[0].classList.contains('paragraph-test')).toBeTruthy();
    expect(
      wrapper.find('.ddmc-typography')[0].classList.contains('ddmc-typography-spacing-close')
    ).toBeTruthy();
  });
  ...
  it('support copyable', async () => {
    ...
    expect(wrapper.find('.ddmc-icon-copy')).toHaveLength(1);

    act(() => {
      fireEvent.click(wrapper.find('.ddmc-typography-operation-copy')[0]);
    });
    ...
    expect(wrapper.find('.ddmc-icon-copy')).toHaveLength(0);
    expect(wrapper.find('.ddmc-icon-check-circle-fill')).toHaveLength(1);
  });
  ...
  it('support editable correctly', () => {
    ...
    act(() => {
      fireEvent.click(wrapper.find('.ddmc-typography-operation-edit')[0]);
    });

    expect(onStart).toHaveBeenCalledTimes(1);
    expect(wrapper.find('.ddmc-textarea')).toHaveLength(1);

    act(() => {
      fireEvent.change(wrapper.find('.ddmc-textarea')[0], { target: { value: afterText } });
    });
    expect(onChange.mock.calls[0][0]).toEqual(afterText);
    act(() => {
      fireEvent.blur(wrapper.find('.ddmc-textarea')[0]);
      wrapper.rerender(<Paragraph editable={{ onChange, onStart, onEnd }}>{afterText}</Paragraph>);
    });
    expect(onEnd).toHaveBeenCalledTimes(1);
    expect(wrapper.find('.ddmc-typography')[0]).toHaveTextContent(afterText);
  });
});
