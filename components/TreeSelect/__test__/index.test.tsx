...
const prefixCls = '.ddmc-tree';

const clickInput = () => {
  fireEvent.click(document.querySelector('.ddmc-tree-select-view') as Element);
};
...
describe('TreeSelect', () => {
  ...
  it('search operation with inputValue', () => {
    ...
    expect(component.find('.ddmc-tree-node')).toHaveLength(2);
  });
  ...
  it('popOver expand and collapse', () => {
    ...
    clickInput();
    expect(wrapper.find('.ddmc-tree')).toHaveLength(1);
    ...
    const text = $('.ddmc-tree-node-title-text').item(0).innerHTML;
    fireEvent.click($('.ddmc-tree-node-title').item(0) as HTMLDivElement);

    jest.runAllTimers();
    expect(wrapper.find('.ddmc-tree')).toHaveLength(0);
    expect($('.ddmc-tree-select-view-value').item(0).innerHTML).toBe(text);
  ...
  it('select correctly', () => {
    ...
    expect(wrapper.querySelector('.ddmc-tree-select-view-value')?.textContent).toBe('小恶魔');
  ...
  it('be controlled', () => {
    ...
    expect(wrapper.querySelector('.ddmc-tree-select-view-value')).toHaveTextContent('史塔克家族');
  });
  ...
  it('search operation', () => {
    ...
    expect(wrapper.find('.ddmc-tree-node')).toHaveLength(2);
  });
  ...
  it('labelInValue props', () => {
    ...
    const choice = wrapper.find('.ddmc-tag');
    ...
    expect(wrapper.find('.ddmc-tag').item(0).textContent).toBe('ceshi');
  });
  ...
  it('allowClear props', () => {
    ...
    fireEvent.click(wrapper.find(`.ddmc-icon-close`).item(0));
  ...
  describe('treeCheckable', () => {
    it('checkable correctly', () => {
      ...
      expect(wrapper.find('.ddmc-checkbox')).toHaveLength(wrapper.find('.ddmc-tree-node').length);
    ...
    it('treeCheckStrategy=all correctly', async () => {
      ...
      expect(wrapper.find('.ddmc-tree-select-view .ddmc-tag')).toHaveLength(4);

      fireEvent.click(document.querySelectorAll('.ddmc-tag .ddmc-icon-close')[0]);

      expect(stateValue).toEqual([]);

      expect(wrapper.find('.ddmc-tree-select-view .ddmc-tag')).toHaveLength(0);
    });
    ...
    it('checkable correctly when checkStrategy is parent', () => {
      ...
      expect(wrapper.find('.ddmc-tag')).toHaveLength(1);
      expect(wrapper.find('.ddmc-tag').item(0).textContent).toEqual('拉尼斯特家族');
      ...
      expect(wrapper.find('.ddmc-tag').item(1).textContent).toBe('三傻');
      ...
      expect(wrapper.find('.ddmc-tag')).toHaveLength(2);
      expect(wrapper.find('.ddmc-tag').item(1).textContent).toBe('史塔克家族');
    });
    ...
    //   await sleep(20);
    //   // TODO: fix this
    //   // expect(wrapper.find('.ddmc-tag')).toHaveLength(5);
    //   tree
    //     .find(`${prefixCls}-node`)
    //     .last()
    //     .find(`${prefixCls}-node-title`)
  ...
  it('test remove item', () => {
    ...
    const removeIcon = wrapper.find('.ddmc-tag .ddmc-icon-close');

    const tags = wrapper.find('.ddmc-tag');
  ...
  it('onSearch ', () => {
    ...
    // 小恶魔
    fireEvent.click(wrapper.find('.ddmc-tree-node-title').item(1));
    ...
    expect(wrapper.find('.ddmc-tree-node')).toHaveLength(2);
    expect(wrapper.querySelector('.ddmc-tag')?.textContent).toBe('小恶魔');
    ...
    expect(wrapper.find('.ddmc-tag')).toHaveLength(2);
    expect(wrapper.querySelectorAll('.ddmc-tag').item(0)?.textContent).toBe('小恶魔');
    expect(wrapper.querySelectorAll('.ddmc-tag').item(1)?.textContent).toBe('二丫');
  });
  ...
  it('renderFormat correctly', () => {
    ...
    expect(wrapper.find('.ddmc-tree-select-view-value').item(0).textContent).toBe(
      `-- ${treeData[0].title}`
    );
    fireEvent.click(wrapper.querySelector('.ddmc-tree-select-view') as HTMLElement);

    fireEvent.click(wrapper.find('.ddmc-tree-node-title').item(2) as HTMLElement);
