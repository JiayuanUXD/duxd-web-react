...
describe('form item children funtion', () => {
  it('default', async () => {
    ...
    expect(wrapper.container.querySelectorAll('.ddmc-form-label-item')).toHaveLength(1);

    formRef.setFieldsValue({ type: '类型A' });

    expect(wrapper.container.querySelectorAll('.ddmc-form-label-item label')[1].innerHTML).toBe(
      ' 类型A名字'
    );
    expect(wrapper.container.querySelector('.ddmc-textarea')?.getAttribute('placeholder')).toBe(
      '类型A的备注'
    );

    fireEvent.change(wrapper.querySelector('.ddmc-input') as Element, { target: { value: '123' } });
    ...
    fireEvent.click(wrapper.querySelector('.ddmc-select-view') as Element);
    fireEvent.click(wrapper.find('.ddmc-select-option')[1]);
    expect(formRef.getFieldsValue()).toEqual({ type: '类型B', nameB: 'B2' });

    formRef.setFieldsValue({ type: '类型A' });

    expect(wrapper.querySelector('.ddmc-input')?.getAttribute('value')).toBe('123');
...
describe('form usewatch ', () => {
  ...
  it('form item disabled', () => {
    ...
    expect(wrapper.find('.ddmc-input-disabled')).toHaveLength(0);
    expect(wrapper.find('.ddmc-input')).toHaveLength(4);
  });
});
