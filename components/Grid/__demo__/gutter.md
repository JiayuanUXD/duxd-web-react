...
```js
import { Grid, Divider } from '@ddmc-design/web-react';
const Row = Grid.Row;
const Col = Grid.Col;
...
```css
.grid-gutter-demo .ddmc-col {
  height: 48px;
  color: var(--color-white);
}

.grid-gutter-demo .ddmc-col > div {
  display: flex;
  align-items: center;
...
.grid-gutter-demo .ddmc-col:nth-child(2n + 1) > div {
  background-color: rgba(var(--ddmcgreen-6), 0.9);
}

.grid-gutter-demo .ddmc-col:nth-child(2n) > div {
