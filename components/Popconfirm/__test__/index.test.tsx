...
describe('Popconfirm', () => {
  ...
  it('render correctly', () => {
    ...
    expect(component.find('.ddmc-popconfirm').length).toBe(0);

    fireEvent.click(triggerEle);

    expect(component.find('.ddmc-popconfirm').length).toBe(1);
    expect(component.find('.ddmc-popconfirm-title-text')[0].innerHTML).toBe('Are you OK?');
  });
  ...
  it('onOk and onCancel correctly', async () => {
    ...
    expect($('.ddmc-popconfirm').length).toBe(1);
    ...
    expect($('.ddmc-popconfirm').length).toBe(0);
