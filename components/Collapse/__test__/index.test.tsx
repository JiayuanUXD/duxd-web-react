...
describe('Collapse', () => {
  ...
  it('collapse expandIcon', () => {
    ...
    expect(
      wrapper.find(`${prefixCls}-item`).item(0).querySelectorAll('.ddmc-icon-plus')
    ).toHaveLength(1);
    expect(
      wrapper.find(`${prefixCls}-item`).item(1).querySelectorAll('.ddmc-icon-plus')
    ).toHaveLength(0);
    expect(wrapper.find(`${prefixCls}-item`).item(1).className).toContain(
      'ddmc-collapse-item-no-icon'
    );
  });
  ...
  it('render correctly when content empty', () => {
    ...
    expect(wrapper.find('.ddmc-collapse')).toHaveLength(1);
    expect(wrapper.find('.ddmc-collapse-item')).toHaveLength(0);
  });
  ...
  it('render self header correctly', () => {
    ...
    expect(wrapper.find('.ddmc-icon-info-circle')).toHaveLength(data.length);
  });
  ...
  it('expand and collapse correctly when set disabled', () => {
    ...
    expect(wrapper.find(`${prefixCls}-item-header`).item(disabledIndex).className).toContain(
      'ddmc-collapse-item-header-disabled'
    );
  });
  ...
  it('render extra correctly', () => {
    ...
    expect(wrapper.find('.ddmc-icon-info-circle')).toHaveLength(data.length);
    expect(wrapper.find('.ddmc-icon-plus')).toHaveLength(data.length);
    expect(wrapper.find('.ddmc-collapse-item-header').item(0).className).toContain(
      'ddmc-collapse-item-header-right'
    );
  });
  ...
  it('render lazyload correctly', () => {
    ...
    expect(wrapper.find('.ddmc-collapse-item-content')).toHaveLength(1);
    fireEvent.click(wrapper.find(`${prefixCls}-item-header`).item(1));
    expect(wrapper.find('.ddmc-collapse-item-content')).toHaveLength(2);
  });
  ...
  it('expand and collapse correctly with trigger region', () => {
    ...
    expect(activeKey).toHaveLength(1);
    fireEvent.click(wrapper.querySelector('.ddmc-icon-hover'));
    expect(activeKey).toHaveLength(0);
  });
});
