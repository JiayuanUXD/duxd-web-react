...
describe('Checkbox group', () => {
  it('Checkbox default options', () => {
    ...
    expect(wrapper.find('.ddmc-checkbox-group')[0]).toHaveClass(
      'ddmc-checkbox-group-direction-vertical'
    );
  });
  ...
  it('Checkbox default options object', () => {
    ...
    expect(
      [].slice.call(wrapper.find('.ddmc-checkbox-text')).map((x) => {
  ...
  it('CheckboxGroup onChange', () => {
    ...
    fireEvent.click(wrapper.find('.ddmc-checkbox').item(0));

    expect(mockFn.call.length).toBe(1);

    expect(wrapper.find('.ddmc-checkbox').item(0)).toHaveClass('ddmc-checkbox-checked');
  });
});
...
describe('checkbox group controled', () => {
  it('defaultvalue', () => {
    ...
    expect(wrapper.find('.ddmc-checkbox').item(1)).toHaveClass('ddmc-checkbox-checked');
    // checked: true,
    fireEvent.click(wrapper.find('.ddmc-checkbox').item(0));
    expect(mockFn.call.length).toBe(1);
  ...
  it('value', () => {
    ...
    fireEvent.click(wrapper.find('.ddmc-checkbox > input').item(1));
    ...
    expect(
      wrapper.find('.ddmc-checkbox')[0].className.includes('ddmc-checkbox-checked')
    ).toBeFalsy();
    expect(wrapper.find('.ddmc-checkbox').item(1)).toHaveClass('ddmc-checkbox-checked');
  });
  ...
  it('value is undefined', () => {
    ...
    expect(wrapper.find('.ddmc-checkbox-checked')).toHaveLength(0);

    rerender({ value: [options[0]] });
    expect(wrapper.find('.ddmc-checkbox-checked')).toHaveLength(1);
...
describe('checkbox group children', () => {
  ...
  fireEvent.click(wrapper.find('.ddmc-checkbox').item(1));
  fireEvent.click(wrapper.find('.ddmc-checkbox').item(2));

  expect(checked).toEqual(['Option 1', 'Option 3']);
  wrapper.rerender(<Demo value={checked} />);
  expect(wrapper.find('.ddmc-checkbox').item(2)).toHaveClass('ddmc-checkbox-checked');
  ...
  expect(
    wrapper.find('.ddmc-checkbox').item(0).className.includes('ddmc-checkbox-checked')
  ).toBeFalsy();
  expect(wrapper.find('.ddmc-checkbox').item(2)).toHaveClass('ddmc-checkbox-checked');
