...
const getNodesContent = (nodes) => {
  return [].slice.call(nodes).map((x) => x.querySelector('.ddmc-tree-node-title-text').textContent);
};

const getCheckedKeys = () => {
  return [].slice
    .call(document.querySelectorAll('.ddmc-checkbox-checked'))
...
describe('Tree', () => {
  ...
  it('tree render treedata', () => {
    ...
    expect(wrapper.querySelector('.ddmc-tree')?.innerHTML).toEqual(
      wrapper2.querySelector('.ddmc-tree')?.innerHTML
    );
  });
  ...
  it('should be expanded and collapsed', async () => {
    ...
    expect(wrapper.find('.ddmc-tree-node').item(0)).toHaveClass('ddmc-tree-node-expanded');
    ...
    expect(
      wrapper.find('.ddmc-tree-node').item(0).classList.contains('ddmc-tree-node-expanded')
    ).toBeFalsy();
    ...
    expect(wrapper.find('.ddmc-tree-node').item(1)).toHaveClass('ddmc-tree-node-expanded');
  });
  ...
  it('should be expanded and collapsed when expandedKeys in props', async () => {
    ...
    expect(wrapper.find('.ddmc-tree-node-expanded')).toHaveLength(2);
    expect(getNodesContent(wrapper.querySelectorAll('.ddmc-tree-node-expanded'))).toEqual([
      '史塔克家族',
      '二丫',
    ]);
  });
  ...
  it('should be selected correctly', () => {
    ...
    expect(wrapper.find('.ddmc-tree-node-selected')).toHaveLength(1);

    expect(wrapper.find('.ddmc-tree-node').item(3)).toHaveClass('ddmc-tree-node-selected');
    fireEvent.click(wrapper.find(`${prefixCls}-node-title`).item(0));
    expect(mockSelected.mock.calls).toHaveLength(1);

    expect(wrapper.find('.ddmc-tree-node-selected')).toHaveLength(1);
    expect(wrapper.find('.ddmc-tree-node').item(0)).toHaveClass('ddmc-tree-node-selected');
  });
  it('should be selected correctly when multiple', () => {
    ...
    expect(wrapper.find('.ddmc-tree-node-selected')).toHaveLength(2);
    expect(wrapper.find('.ddmc-tree-node').item(0)).toHaveClass('ddmc-tree-node-selected');
    expect(wrapper.find('.ddmc-tree-node').item(3)).toHaveClass('ddmc-tree-node-selected');
  });
  ...
  it('should checked correctly', async () => {
    ...
    fireEvent.click(
      wrapper.find(`${prefixCls}-node`).item(3).querySelector(`.ddmc-checkbox`) as any
    );
  ...
  it('self icon render correctly', () => {
    ...
    expect(wrapper.find('.ddmc-icon-heart-fill')).toHaveLength(1);
  ...
  it('should selectable, disableCheckbox correctly', () => {
    ...
    fireEvent.click(nodes[nodes.length - 1].querySelector(`.ddmc-checkbox`) as any);
  ...
  it('should checkedStrategy correctly', async () => {
    ...
    fireEvent.click(wrapper.find(`.ddmc-checkbox`).item(3));
    ...
    expect(wrapper.find('.ddmc-checkbox.ddmc-checkbox-checked')).toHaveLength(2);
  });
  ...
  it('should halfchecked correctly', () => {
    ...
    expect(wrapper.find(`${prefixCls}-node`).item(0).querySelector(`.ddmc-checkbox`)).toHaveClass(
  ...
  it('should be expanded when click node', () => {
    ...
    expect(wrapper.find('.ddmc-tree-node-title-text').item(0).textContent).toBe('拉尼斯特家族');

    fireEvent.click(wrapper.find(`${prefixCls}-node-title`).item(0));
    expect(mockExpand.mock.calls).toHaveLength(1);
    expect(mockSelected.mock.calls).toHaveLength(0);
    expect(wrapper.find('.ddmc-tree-node-title-text').item(1).textContent).toBe('小恶魔');
  });
  ...
  it('should be checked and expand when click node', () => {
    ...
    fireEvent.click(wrapper.find(`${prefixCls}-node-title`).item(0));
    expect(wrapper.find('.ddmc-tree-node-title-text').item(1).textContent).toBe('小恶魔');
    expect(wrapper.find('.ddmc-checkbox-checked')).toHaveLength(2);
