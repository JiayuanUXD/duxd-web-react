...
class Tree extends Component<TreeProps, TreeState> {
  ...
  getCacheNode = (key: string[]): NodeInstance[] => {
    // __DdmcAdapterMode__ ，在大数据下获取选中结点信息时，构建 Node 会导致性能消耗过大，
    // 通过 __DdmcAdapterMode__ 开启构建数据一个类似 node 的数据结构，提升性能，大版本升级时调整所有回调类型的 NodeInstance 的参数
    const { __DdmcAdapterMode__ } = this.props;
    const originData = [];
