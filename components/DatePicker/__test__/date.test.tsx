...
describe('DatePicker', () => {
  ...
  it('dayStartOfWeek', () => {
    ...
    expect(component.find('.ddmc-picker-week-list-item').item(0).textContent).toBe('一');
    component.rerender(<DatePicker dayStartOfWeek={0} />);

    expect(component.find('.ddmc-picker-week-list-item').item(0).textContent).toBe('日');
    component.rerender(<DatePicker dayStartOfWeek={6} />);

    expect(component.find('.ddmc-picker-week-list-item').item(0).textContent).toBe('六');
  });

  it('defaultValue', () => {
    const component = render(<DatePicker defaultValue="2020-02-02" />);

    expect(component.find('.ddmc-picker-input input')[0].getAttribute('value')).toBe('2020-02-02');

    fireEvent.click(component.container.firstChild!);

    fireEvent.click(
      component.find('.ddmc-picker-date').item(8) // 2020-02-04
    );
  ...
  it('today', () => {
    ...
    fireEvent.click(component.find('.ddmc-picker-footer .ddmc-link')[0]);

    expect(component.find('.ddmc-picker-input input')[0].getAttribute('value')).toBe('2020-04-10');
  });
  ...
  it('control mode (showTime)', async () => {
    ...
    fireEvent.click(component.find('button.ddmc-picker-btn-select-time')[0]);
  ...
  it('showTime', async () => {
    ...
    function getTimePickerCell(index, cellIndex) {
      return component
        .find('.ddmc-timepicker-list')
        .item(index)
        .querySelectorAll('.ddmc-timepicker-cell')
        .item(cellIndex);
    }

    function getInputValue() {
      return component.find('.ddmc-picker-input input')[0].getAttribute('value');
    }
    fireEvent.click(component.find('button.ddmc-picker-btn-select-time')[0]);
    ...
    fireEvent.click(component.find('.ddmc-picker-footer-btn-wrapper .ddmc-btn-primary')[0]);

    expect(getInputValue()).toBe('2020-04-10 10:11:12');

    // click now btn
    fireEvent.click(component.find('.ddmc-picker')[0]);

    await sleep(1000);
    fireEvent.click(component.queryByText('选择时间')!);

    fireEvent.click(component.find('.ddmc-picker-footer-btn-wrapper .ddmc-btn-secondary')[0]);
  ...
  it('hideNotInViewDates', () => {
    ...
    expect(component.find('.ddmc-picker-cell-hidden')).toHaveLength(12);
  });

  it('update value', () => {
    const component = render(<DatePicker value="2020-04-01" />);

    expect(component.find('.ddmc-picker-input input')[0].getAttribute('value')).toBe('2020-04-01');
    component.rerender(<DatePicker value="2020-05-06" />);

    expect(component.find('.ddmc-picker-input input')[0].getAttribute('value')).toBe('2020-05-06');
  });
  ...
  it('hover placeholder', async () => {
    ...
    function checkPlaceholder(isPlaceholder: boolean) {
      expect(
        getInput(component, 0).parentElement!.classList.contains('ddmc-picker-input-placeholder')
      ).toBe(isPlaceholder);
    }
  ...
  it('shortcuts', async () => {
    ...
    expect(getInput(component, 0).getAttribute('value')).toBe('');
    expect(component.find('.ddmc-picker-cell-selected')).toHaveLength(0);
    ...
    // 2020-04-11
    expect(
      getDateCell(component, 0, 12).classList.contains('ddmc-picker-cell-selected')
    ).toBeTruthy();
    expect(component.find('.ddmc-picker-cell-selected')).toHaveLength(1);
  ...
  it('onSelect & onChange (showTime)', async () => {
    ...
    // confirm
    fireEvent.click(component.find('button.ddmc-picker-btn-confirm')[0]);
