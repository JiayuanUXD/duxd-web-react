...
describe('RangePicker', () => {
  ...
  it('hideNotInViewDates', () => {
    ...
    expect(component.find('.ddmc-picker-cell-hidden')).toHaveLength(23);
  });
  ...
  it('hover placeholder', async () => {
    ...
    expect(
      getInput(component, 0).parentElement?.classList.contains('ddmc-picker-input-active')
    ).toBeTruthy();

    fireEvent.click(getDateCell(component, 0, 7).querySelector('.ddmc-picker-date')!);

    expect(
      getInput(component, 1).parentElement?.classList.contains('ddmc-picker-input-active')
    ).toBeTruthy();
    ...
    // 2020-04-07: click
    fireEvent.click(getDateCell(component, 0, 8).querySelector('.ddmc-picker-date')!);
  ...
  it('shortcuts', async () => {
    ...
    expect(
      getDateCell(component, 0, 11).classList.contains('ddmc-picker-cell-range-start')
    ).toBeTruthy();
    expect(
      getDateCell(component, 0, 18).classList.contains('ddmc-picker-cell-range-end')
    ).toBeTruthy();
  ...
  it('clearRangeOnReselect', async () => {
    ...
    // 2020-04-11
    fireEvent.click(getDateCell(component, 0, 13).querySelector('.ddmc-picker-date')!);
    ...
    // 2020-04-12
    fireEvent.click(getDateCell(component, 0, 14).querySelector('.ddmc-picker-date')!);

    expect(onChange.mock.calls.length).toBe(1);

    // 2020-04-13
    fireEvent.click(getDateCell(component, 0, 15).querySelector('.ddmc-picker-date')!);
  ...
  // issue: https://github.com/ddmc-design/ddmc-design/issues/1570
  it('change mode', async () => {
    ...
    function getFirstPickerHeaderValue(component) {
      return component.find('.ddmc-picker-header-value').item(0).textContent;
    }

    function getSecondPickerHeaderValue(component) {
      return component.find('.ddmc-picker-header-value').item(1).textContent;
    }
    ...
    expect(component.find('.ddmc-picker-cell-week')).toHaveLength(12);
  });

  it('separator', () => {
    const component = render(<RangePicker separator="to" popupVisible />);

    expect(component.find('.ddmc-picker-separator')[0].textContent).toBe('to');
  });
  ...
  it('change disabled', () => {
    ...
    expect(
      component.find('.ddmc-picker-input')[1].classList.contains('ddmc-picker-input-active')
    ).toBeTruthy();

    expect(getDateCell(component, 0, 1).className).toBe(
      'ddmc-picker-cell ddmc-picker-cell-disabled'
    );
