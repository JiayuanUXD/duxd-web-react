...
describe('Modal', () => {
  ...
  it('closeIcon correctly', () => {
    ...
    expect(component.querySelector('.ddmc-modal-close-icon')?.textContent).toBe('xxx');
  });

  it('open modal correctly', () => {
    const wrapper = render(<DemoTest />);
    expect(wrapper.find('.ddmc-modal')).toHaveLength(0);
    // modal mask correctly
    expect($('.ddmc-modal-mask').length).toBe(0);

    fireEvent.click(wrapper.queryByText('Open') as Element);

    expect(wrapper.find('.ddmc-modal')).toHaveLength(1);
    expect(wrapper.querySelector('.ddmc-modal-wrapper')).toHaveStyle('display: block');

    expect($('.ddmc-modal-mask').length).toBe(1);

    const closeIcon = wrapper.querySelector('.ddmc-modal-close-icon');
    expect(closeIcon).toBeTruthy();
    fireEvent.click(closeIcon as Element);

    jest.runAllTimers();

    expect(wrapper.querySelector('.ddmc-modal-wrapper')).toHaveStyle('display: none');
    expect($('.ddmc-modal-mask').length).toBe(1);
  });
  ...
  it('onConfirm and onCancel correctly', () => {
    ...
    open();
    expect($('.ddmc-modal-wrapper')[0].style.display).toBe('block');
    fireEvent.click(wrapper.queryByText('确定') as Element);
    jest.runAllTimers();
    expect($('.ddmc-modal-wrapper')[0].style.display).toBe('none');
  ...
  it('close Modal with esc keydown event and focusLock is false', () => {
    ...
    jest.runAllTimers();
    expect(document.querySelectorAll(`.ddmc-modal-wrapper`)).toHaveLength(2);
    fireEvent.keyDown(document.querySelectorAll('.ddmc-modal-wrapper')[0], {
      key: Esc.key,
    });
    jest.runAllTimers();
    expect(document.querySelectorAll(`.ddmc-modal-wrapper`)).toHaveLength(1);
  ...
  it('clear Modal dom ', () => {
    ...
    jest.runAllTimers();
    expect(document.querySelectorAll(`.ddmc-modal-wrapper`)).toHaveLength(0);
    jest.useRealTimers();
  });
  ...
  it('should keep focus on DatePicker input and close modal with esc', () => {
    ...
    const dateInput = document.querySelector('.ddmc-picker-input input') as HTMLInputElement;
