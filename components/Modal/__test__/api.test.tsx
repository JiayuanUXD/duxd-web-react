...
describe('Modal api test', () => {
  ...
  it('simple and closable', () => {
    ...
    expect($('.ddmc-modal-close-icon').length).toBe(1);

    wrapper.rerender(<Modal visible simple />);
    expect($('.ddmc-modal-close-icon').length).toBe(0);
  ...
  it('click mask', () => {
    ...
    jest.useFakeTimers();
    act(() => {
      fireEvent.mouseDown(wrapper.find('.ddmc-modal-wrapper')[0]);
      fireEvent.click(wrapper.find('.ddmc-modal-wrapper')[0]);
    });
  ...
  it('modalRender and custom footer', () => {
    ...
    // 作为modal的兄弟节点
    expect(document.querySelectorAll('.ddmc-modal+.test-content')).toHaveLength(1);

    expect(wrapper.find('.ddmc-modal-footer')[0].innerHTML).toBe('<div>1234</div>');
  });
});
