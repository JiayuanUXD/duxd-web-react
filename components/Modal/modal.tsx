...
function Modal(baseProps: PropsWithChildren<ModalProps>, ref) {
  ...
  useEffect(() => {
    ...
    if (escToExit) {
      timer = setTimeout(() => {
        // https://github.com/ddmc-design/ddmc-design/pull/1439
    ...
    >
      {title && (
        <div className={`${prefixCls}-header`}>
          <div className={`${prefixCls}-title`} id={`ddmc-dialog-${dialogIndex.current}`}>
            {title}
          </div>
        </div>
      )}
  ...
  const ariaProps = title ? { 'aria-labelledby': `ddmc-dialog-${dialogIndex.current}` } : {};
