...
const changePageSize = (component, index: number) => {
  const pageSizeChanger = component.find('.ddmc-select-single')[0];
  fireEvent.click(pageSizeChanger);
  const pageSizeChangeOptions = document.getElementsByClassName('ddmc-select-option');
  if (index < 0) {
...
describe('Pagination', () => {
  it('pagination should be update correctly', () => {
    ...
    const liList = component.find<HTMLLIElement>('.ddmc-pagination-item');
  ...
  it('pagination should jump correctly pages', () => {
    ...
    expect(component.find('.ddmc-pagination-item-active').item(0).innerHTML).toBe('1');
    const PageJumperInput = component.querySelector<HTMLInputElement>(
      '.ddmc-pagination-jumper-input'
    );
    fireEvent.change(PageJumperInput!, { target: { value: 30 } });
    fireEvent.keyDown(PageJumperInput!, { keyCode: Enter.code });
    expect(component.querySelector('.ddmc-pagination-item-active')!.innerHTML).toBe('20');
  });
  ...
  it('should change pageSize correctly', () => {
    ...
    expect(component.find('.ddmc-pagination-item')).toHaveLength(Math.ceil(200 / 30) + 2);
    ...
    expect(component.find('.ddmc-pagination-item')).toHaveLength(2);
    expect(component.find('.ddmc-pagination-item-disabled')).toHaveLength(2);
  });
  ...
  it('should display defaultPageSize when sizeCanChange is enabled', () => {
    ...
    expect(component.find('.ddmc-select-view-value')[0].innerHTML.startsWith('20')).toBe(true);
  });
  ...
  it('should pageSizeChangeResetCurrent work', () => {
    ...
    const changeCurrentPageItems: Array<HTMLLIElement> = Array.apply(
      null,
      component.find('.ddmc-pagination-item')
    );
    changeCurrentPageItems.pop();
    changeCurrentPageItems.shift();
    fireEvent.click(changeCurrentPageItems[2]);
    expect(component.find('.ddmc-pagination-item-active')[0].innerHTML).toBe('3');
    changePageSize(component, 1);
    expect(component.find('.ddmc-pagination-item-active')[0].innerHTML).toBe('1');
  });

  it('fold page correctly when set bufferSize', () => {
    const component = render(<Pagination total={100} current={5} bufferSize={1} />);
    expect(component.find('.ddmc-pagination-item-jumper')).toHaveLength(2);
    // < + begin + ...  + bufferSize + current + bufferSize + ... + end + > = 9
    expect(component.find('.ddmc-pagination-item')).toHaveLength(9);

    component.rerender(<Pagination total={100} current={2} bufferSize={1} />);
    expect(component.find('.ddmc-pagination-item-jumper')).toHaveLength(1);
  });

  it('show jumper correctly in simple mode', () => {
    const component = render(<Pagination simple total={100} current={5} />);
    expect(component.find<HTMLInputElement>('.ddmc-pagination-jumper-input')[0].value).toEqual('5');
    component.rerender(<Pagination simple total={100} current={5} showJumper={false} />);
    expect(component.find('.ddmc-pagination-jumper-input')).toHaveLength(0);
  });
  ...
  it('sizeOptions changed correctly', () => {
    ...
    const lastPageOptionIndex = component.find('.ddmc-pagination-item').length - 2;
    expect(component.find('.ddmc-pagination-item')[lastPageOptionIndex].innerHTML).toEqual(
      `${Math.ceil(total / sizeOptions1[0])}`
    );
    act(async () => {
      fireEvent.click(component.find('.ddmc-select')[0]);
      await sleep(1000);
    });

    act(() => {
      fireEvent.click(component.find('.ddmc-select-option')[2]);
    });

    const totalLength = Math.ceil(total / sizeOptions1[2]) + 2;
    expect(component.find('.ddmc-pagination-item').length).toEqual(totalLength);
    expect(component.find('.ddmc-pagination-item-active')[0].innerHTML).toEqual('1');

    act(() => {
      fireEvent.click(component.find('.btn-trigger')[0]);
      fireEvent.click(component.find('.ddmc-pagination-item')[totalLength - 3]);
    });

    const currentActive = component.find('.ddmc-pagination-item-active')[0].innerHTML;
    expect(
      component.find('.ddmc-select-view-value')[0].innerHTML.startsWith(sizeOptions2[0].toString())
    ).toBe(true);
    ...
    act(() => {
      fireEvent.click(component.find('.ddmc-select-option')[1]);
    });
    expect(component.find('.ddmc-pagination-item-active')[0].innerHTML).toEqual(currentActive);

    act(() => {
      fireEvent.click(component.find('.ddmc-select-option')[3]);
    });
    expect(component.find('.ddmc-pagination-item-active')[0].innerHTML).toEqual(
  ...
  it('illegal props are behaving normally', () => {
    ...
    expect(wrapper.find('.ddmc-pagination-item')).toHaveLength(2 + 30 / 5);
    act(() => {
      fireEvent.click(wrapper.find('button')[0]);
    });

    expect(wrapper.find('.ddmc-pagination-item')).toHaveLength(2);
  });
});
