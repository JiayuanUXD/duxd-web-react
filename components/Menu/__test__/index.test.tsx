...
describe('Menu', () => {
  it('should render correct Menu and children based on vertical props', () => {
    ...
    expect(wrapper.querySelectorAll('.ddmc-menu-inner > *').length).toBe(5);
    expect(wrapper.querySelector('.ddmc-menu')).toHaveClass('ddmc-menu-test');
  });

  it('should render correct theme', () => {
    const wrapper = render(generateMenu({ theme: 'dark' }));
    expect(wrapper.querySelector('.ddmc-menu')).toHaveClass('ddmc-menu-dark');
  });

  it('should render menuItem disabled', () => {
    const wrapper = render(generateMenu());
    expect(wrapper.querySelectorAll('.ddmc-menu-disabled').length).toBe(1);
  });

  it('click items should change active and call the right callback', () => {
    const wrapper = render(generateMenu(testProps));
    expect(wrapper.querySelector('.ddmc-menu-selected')).toHaveTextContent('设计指南');
    fireEvent.click(wrapper.querySelectorAll('.ddmc-menu-item')[2]);
    expect(testProps.onClickMenuItem).toBeCalled();
    expect(wrapper.querySelector('.ddmc-menu-selected')).toHaveTextContent('模块');
  });

  it('vertical openKeys', () => {
    const wrapper = render(generateMenu(testProps));
    expect(getComputedStyle(wrapper.querySelector('.ddmc-menu-inline-content')).height).toBe(
      'auto'
    );
    fireEvent.click(wrapper.querySelector('.ddmc-menu-inline-header'));
    expect(testProps.onClickSubMenu).toBeCalled();
    expect(getComputedStyle(wrapper.querySelector('.ddmc-menu-inline-content')).height).toBe('0px');
  });
  ...
  it('triggerProps works', async () => {
    ...
    const popupMenuItems = wrapper.querySelectorAll('.ddmc-dropdown-menu-item');
  ...
  it('accordion', () => {
    ...
    const eleInlineContents = wrapper.querySelectorAll('.ddmc-menu-inline-content');
    expect(getComputedStyle(eleInlineContents[0]).height).toBe('auto');

    fireEvent.click(wrapper.querySelectorAll('.ddmc-menu-inline-header')[1]);
    expect(testProps.onClickSubMenu).toBeCalled();
  ...
  it('ItemGroup', () => {
    ...
    expect(wrapper.querySelectorAll('.ddmc-menu-group').length).toBe(1);
  });
  ...
  it('horizontal', async () => {
    ...
    fireEvent.click(wrapper.querySelector('.ddmc-menu-pop-header'));
    await sleep(0);
    expect(wrapper.querySelectorAll('.ddmc-menu-pop-trigger').length).toBe(1);

    expect(testProps.onClickSubMenu).toBeCalled();

    fireEvent.click(wrapper.querySelector('.ddmc-menu-pop-header'));
    await sleep(100);
    expect(wrapper.querySelectorAll('.ddmc-menu-pop-trigger').length).toBe(0);
  });
  ...
  it('collapse', async () => {
    ...
    expect(wrapper.querySelectorAll('.ddmc-dropdown-menu').length).toBe(1);
  ...
  it('SubMenu properties are passed in correctly', () => {
    ...
    expect(wrapper.querySelectorAll('.ddmc-menu-item-inner')).toHaveLength(2);
    expect(wrapper.querySelector('.ddmc-menu-pop-header')).toHaveTextContent('sub_submenu_2');
  });
  ...
  it('SubMenu selectable', () => {
    ...
    expect(wrapper.querySelectorAll('.ddmc-menu-selected')).toHaveLength(1);
  });
});
