...
describe('Anchor', () => {
  it('Anchor to the link correctly', async () => {
    ...
    expect(wrapper.querySelector('a[href="#b"]').parentNode).toHaveClass('ddmc-anchor-link-active');
  ...
  it('Anchor not fixed', () => {
    ...
    expect(wrapper.container.firstElementChild).toHaveClass('ddmc-anchor');
  });
  ...
  it('should unregister link when unmount children', async () => {
    ...
    expect(wrapper.find('.ddmc-anchor-link')).toHaveLength(2);
    wrapper.rerender(<TestAnchor />);
    expect(wrapper.find('.ddmc-anchor-link')).toHaveLength(0);
  });
