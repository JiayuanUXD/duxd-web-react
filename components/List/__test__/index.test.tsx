...
describe('List', () => {
  ...
  it('render item correctly', () => {
    ...
    expect(wrapper.find('.ddmc-list-item')).toHaveLength(3);
  });
  ...
  it('renders empty loading', () => {
    ...
    expect(wrapper.find('.ddmc-list-item')).toHaveLength(0);
    expect(wrapper.find('.ddmc-spin .ddmc-spin')).toHaveLength(1);
  });
  ...
  it('render pagination correctly', () => {
    ...
    const paginationItems = wrapper.find('.ddmc-pagination-item');
  ...
  it('render List with meta', () => {
    ...
    expect(wrapper.find('.ddmc-list-item-meta-avatar')).toHaveLength(3);
    expect(wrapper.find('.ddmc-list-item-action').item(0).querySelectorAll('li')).toHaveLength(2);
  });
  ...
  it('render List with grid', () => {
    ...
    expect(wrapper.find('.ddmc-list-content .ddmc-row').length).toBe(
  ...
  it('List onReachBottom', () => {
    ...
    fireEvent.scroll(wrapper.find('.ddmc-list')[0], { target: { scrollY: 100 } });
  ...
  it('render List which is virtual', () => {
    ...
    expect(wrapper.find('.ddmc-list-item').length).toBeLessThan(itemCount);
  });
  ...
  it(`List pagination property works`, () => {
    ...
    expect(wrapper.find('.ddmc-pagination .ddmc-select')).toHaveLength(1);
    expect(wrapper.find('.ddmc-list-item-content').item(0).innerHTML).toBe('60');
  });
  ...
  it('List render callback has correct parameter:index', () => {
    ...
    expect(wrapper.find('.ddmc-list-item-content').item(itemCount - 1)).toHaveTextContent(
      `${itemCount}`
    );
  });
});
