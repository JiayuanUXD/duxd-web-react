...
function Button(baseProps: DropdownButtonProps, ref) {
  ...
  let leftButton: ReactNode = (
    <DdmcButton disabled={disabled} type={type} size={size} onClick={onClick} {...buttonProps}>
      {children}
    </DdmcButton>
  );
  let rightButton: ReactNode = (
    <DdmcButton disabled={disabled} type={type} size={size} icon={icon} />
  );
  ...
  return (
    <DdmcButton.Group className={className} style={style} ref={ref} {...pickDataAttributes(props)}>
      {leftButton}
      <Dropdown
        disabled={disabled}
        position={position}
