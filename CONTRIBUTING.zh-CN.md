...
我们使用 [Github issues](https://github.com/ddmc-design/ddmc-design/issues) 进行 bug 报告和新 feature 建议。在报告 bug 之前，请确保已经搜索过类似的 [问题](https://github.com/ddmc-design/ddmc-design/issues)，因为它们可能已经得到解答或正在被修复。新问题应通过 [问题助手](https://ddmc.design/issue-helper?repo=ddmc-design) 提交。对于 bug 报告，请包含可用于重现问题的代码。对于新 feature 建议，请指出你想要的更改以及期望的行为。
...
1. 克隆代码仓库
```bash
$ git clone git@github.com:ddmc-design/ddmc-design.git
```
2. 开发环境初始化
```bash
$  cd ddmc-design && yarn run init
# init 命令到底干了什么？
...
└── style
    ...
    └── index.less （组件样式，类名的命名空间为 ddmc-{组件名}）
```
### 辅助函数
