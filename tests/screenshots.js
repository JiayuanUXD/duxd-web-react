...
const Ddmc = require('../lib');
const DdmcHooks = require('../hooks/lib');

// const components = Object.keys(Ddmc).filter((key) => typeof Ddmc[key] === 'object');
...
(async () => {
  ...
  const list = Object.keys(Ddmc)
    .filter((key) => typeof Ddmc[key] === 'object')
    .map((name) => ({
      name: name
        .replace(/([A-Z])/g, '-$1')
        .toLowerCase()
    ...
    .concat(Object.keys(DdmcHooks).map((name) => ({ name, pagePath: '/hooks' })));
  ...
  const genComponentScreenshots = async ({ name, pagePath }) => {
    ...
    await page.evaluate(() => {
      // 隐藏下悬浮按钮，避免截图干扰
      document.querySelector('.ddmc-page + div').style.display = 'none';
    });
